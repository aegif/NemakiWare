  ...
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  ...
  - **受信先のプロファイル行が解釈できないときも 503** です。従来は解釈できない行を
    ...
    **逆に、`enabled` を無効と読むのは mapper が読む形だけになりました** — 真偽値の `false`、
    数値の `0`、文字列の `"false"` / `"False"` / `"FALSE"` (前後の空白は除かれます)、空文字や
    空白だけの文字列、`"null"` (前後の空白は同じく除かれます)、そして明示的な null。
    後ろの 4 つは「値なし」として false になります。`"fAlSe"` のようにそれ以外の綴りの行は、従来
    ...
    コネクタは止めません。
    `enabled` を false と読める壊れた行 (上に挙げた形 — 真偽値の `false`、数値の `0`、
    3 綴りの文字列、空文字や空白のみ、`"null"`、明示的な null) は受信先になり得ないので
    数えません。行が直る (またはその値を読めるノードに切り替わる) まで続く状態なので、
...
- **取込の自動解決 (`profileId` を省いた取込) が、`enabled: false` の行を解釈しなくなり
  ...
  **有効**な行が解釈できない場合は従来どおり 503 です (その行が既定だったかもしれない
  ...
  JSON の `false`、文字列の `"false"`、明示的な null など — で、コネクタ側の一覧と同じ読み方
  です (値が無い行は無効とは読みません。行の `profileId` も同じ読み方で、数値は文字列として
