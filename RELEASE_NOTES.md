...
## 外部取込に capture 境界を入れました

外部取込は**本文をリポジトリの DB に、証拠を共有の `nemaki_lineage` に**書きます。CouchDB に
DB を跨ぐトランザクションは無く、原子性は単一文書の PUT だけです。従来は本文が先だったため、
その間で落ちると**出所を誰も知らない文書**が残り、しかも証拠の発行は握り潰されるので警告すら
出ませんでした。

順序を逆にしました。「これから取り込む」という行を先に書き、業務操作を行い、最後にその行を
完成させます。どこで落ちても行は必ず在ります。
...
- **保持期間は既定で無期限**です (`lineage.capture-boundary.retention.captured.days` /
  `...unresolved.days`)。これらの行は「いつ何を取り込んだか」「何が失敗したか」の証拠なので、
  運用者が明示的に求めない限り製品側では消しません。**代わりに `nemaki_lineage` は取込量に
  比例して増えます** — しかも取込 1 件につき 1 行とは限らず、添付や保存した生 `.eml` は
  それぞれ自分の行を持ちます (添付 5 件のメールで 7 行)。規模が大きい環境では DB サイズを
  監視してください

### 主張しないこと

`UNRESOLVED` の行が意味するのは「取込が始まり、期限までに完了が記録されなかった」までです。
**文書が作られたともられなかったとも言っていません。** また、この変更が保証するのは
「変更の前に証拠の行がある」ことであって、**lineage イベントが外部へ配送されること**では
ありません (発行は従来どおりで、既存の outbox の担当です)。

なお、orchestrator が取込の入口が返った**後**に作る relationship は、まだこの境界の外です。

## 外部取込の完了を「証拠の連鎖」に積むようになりました

外部取込の capture が完了するたびに、**追記専用の証拠台帳**に 1 件記録します。
台帳は前の項目のハッシュを含むので、**後から途中の 1 件だけを書き換えると連鎖が切れます**。

**運用に影響する点を先に書きます。**

- **新しい CouchDB データベース `nemaki_evidence_ledger` が自動で作られます。**
  最初の外部取込のときに、無ければ作成します。バックアップ対象・容量監視の対象に
  加えてください
- **切るスイッチはありません。** 外部取込の経路がある配備では常に有効です
- **本文は入れません。** 台帳は purge されないので、参加者やアドレスを入れると
  消せない場所に個人データを固定することになります。1 件に入るのは:
  - **平文で**: リポジトリ ID・取込 ID (UUID)・発生時刻・順序番号・前件のハッシュ
  - **ハッシュの中だけ**: connectorId / sourceObjectId / 適用済みメタデータの hash
  (取込 ID から後で引けるようにするための識別子は平文で持ちます。
  「不透明なハッシュが 1 個だけ」ではありません)

- **警告は取込ごとに呼び出し元へ返ります。サーバログは WARN が最初の 1 件と
  以後 100 件ごと、その間は INFO** に出ます (障害の開始を告げる行が同じ行 1 万本に
  埋もれないように)。障害が復旧して 1 件でも連鎖に載ると数え直すので、
  **別の障害の 1 件目も必ず WARN に出ます**
- **`GET /core/api/v1/admin/anchor/status` の
  `chainGapsOnThisReplicaSinceStartup`** で、連鎖に載らなかった件数が読めます
  (**JVM 内・レプリカ内の数**です)
- **台帳に書けなくても取込は成功します。** その時点で本文も capture 行も既に
  永続化されているので、2 本目の記録が書けないことで取込を失敗させると
  **その記録が対象にしていた当のものが消えます**。代わりに応答に警告が入ります
- **定期取得 (Slack / Teams / Chatwork / Mattermost / Dropbox / Box / Salesforce /
  IMAP IDLE) と DLQ 再実行の取得サマリには、この警告は出ません。**
  サーバログを見てください

### 主張しないこと

- **遡って積むことはしません。** この機能より前の取込は連鎖に入っていません。
  後から作った証拠は証拠ではないからです
- 連鎖が繋がっていることが言うのは「**台帳に入った記録どうしが繋がっている**」までで、
  「起きたことが全部台帳に在る」ではありません

## 併せて直した既存の不具合

- **dry run が 4 つの入口で実際に書き込んでいました** — mail / note / record / chat の
  プレビューが、メタデータ・生 `.eml`・添付・relationship を実際に作成していました
- **取込の ACL 適用が古いリビジョンで失敗し、しかも黙っていました** — `aclSyncPolicy` が
  `none` / `copy_from_source` のとき、意図した権限が適用されないまま成功と報告されていました
- **relationship の除去が無限ループすることがありました** — 削除が全部失敗する状況で
  同じページを読み続け、取込が止まりませんでした
- **設定が読めなかったとき、それが「無設定」として JVM の生涯キャッシュされていました** —
  起動時に CouchDB が一瞬落ちただけで、その JVM は全 dynamic 設定を未設定として読み続けました。
  併せて、読めなかった設定の上に書き戻して他の設定を消す経路も塞ぎました
- **取込の失敗が「何も起きなかった」と報告され、再取得もされませんでした** — 入口で落ちると
  作成済みの objectId も蓄積した警告も捨てられ、DLQ にも載りませんでした
...
- **admin パスワード変更の部分成功が見えるようになりました** — 複数リポジトリの一部だけ
  更新に成功した場合、応答に DB ごとの結果 (全件) と「旧パスワードのまま残った DB」を
  名指しする warning が入ります (成功判定は従来どおり。warning 内の列挙は長大な場合
  打ち切られますが、全件は per-DB 結果側で判別できます)。
- **QA fixture (`patch.testuser.enabled=true`) の初回 seed が一時障害で失敗した場合に、
  適用済みと誤記録されて二度と再試行されない**問題を修正しました。
- レンディション変換の一時ファイルを POSIX 600 で作成するようにしました (従来は umask
  依存で、共有 /tmp では変換中の添付本体が他ローカルユーザーから読めました)。
- フォルダ移動のロック取得を lock-before-try 形に整理しました (静的解析が検査する
  構造への追随。旧形も実害は無く、未取得ロックへの no-op unlock 経路の除去です)。

## 内部改善

- 起動プローブの状態と CouchDB 版数を単一スナップショットとして公開し、setup 完了
  ゲートも単一読みに変更 (別プローブの state と版数が対になって混ざる形を解消)
- CodeQL アラート 25 件を全件 triage (修正 2 件・理由付き dismiss 23 件)
- 開発用依存の脆弱性解消 (`npm audit` は dev 依存込みで 0 件)
- Playwright CI を 4 シャード化し、既知のハング様式で全体が 3 時間停止する形を解消
- SOAP の手動スモークを 3 サービスの実往復で実施 (恒久テストの追加ではありません)。
  WS クライアント binding が製品コードで未使用であることを確認・記録
