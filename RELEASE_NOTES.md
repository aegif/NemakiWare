...
- **webhook 受信 (`/core/api/v1/ingest-webhook/{connectorId}`) の受信先解決が索引を読まなく
  ...
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  ...
  - **受信先のプロファイル行が解釈できないときも 503** です。従来は解釈できない行を
    ...
    **逆に、`enabled` の文字列で無効と読むのは `"false"` / `"False"` / `"FALSE"` の 3 綴りだけに
    なりました** (mapper が読む綴り。`"fAlSe"` のようにそれ以外の綴りの行は、従来「無効」として
    飛ばしていたのが「読めない行」として扱われます — その行は全体としても逆直列化できないので、
    無効かどうかを確かめられない側に倒しています)。
...
- **識別子が文字列でない行は、起動時の移行で文字列に直されます。** `profileId` /
  ...
  間はこの状態になり得ます。`repositoryId` が文字列でない行は従来どおり移行しません
