...
- **取込プロファイル (`/core/api/v1/admin/import-profiles`) も同じ扱いです。**
  索引再構築中に見えないプロファイルへの `GET` / `PUT` / `DELETE` は、従来の **404** ではなく
  **503** を返します (存在しないプロファイルは従来どおり 404)。コネクタの `GET` も同じです。
  **プロファイル**は確定的 ID の行も旧 ID しか無い行も索引を使わずに読めるので、索引が
  答えられない間もそもそも 200 が返ります。**コネクタの `GET` は違います** — 索引を使わない
  読みで確かめるのは「行が在るかどうか」だけで、行そのものは取り直しません。旧 ID しか無い
  コネクタは、索引が答えられる間は従来どおり 200 ですが、**索引が答えられない間は 404 では
  なく 503** になります (起動時の移行が確定的 ID に書き直すまでの間)。同じ ID の行がその
  リポジトリに 2 つある場合、`PUT` / 所有権移転と `DELETE` (docId 無し) は **409** で、
  先にどちらかを `?docId=` で片付ける必要があります (`POST` はこの状態でも **400** です —
  下の箇条書きと同じで、409 ではありません)。**`GET` は索引が答えられている間は 200 を返します**
  — どちらの行かは索引の返す順に依存します (書ける動詞が全て止まるので、読みだけを止める
  意味が薄いのと、管理 API 全部を索引不要にすると設定 DB の全走査が毎回入るため)。
  **索引が答えられないときの `GET` は、プロファイルでは 409** — 呼び出し元リポジトリの行が
  どれか索引に訊けず、行が 2 つあることだけが分かるためで、そこで 1 つ選ぶことはしません。
  **コネクタでは 503** です (コネクタはリポジトリで分かれないので、答えは「読めなかった」に
  なります)。
...
- **「読めなかった」が 500 や 400 で返っていた入口を 503 に揃えました。**
  ...
  **確定的 ID に別の文書が居て行をコネクタとして読めない**場合に Spring の **500** を
  返していました (「こちらの不具合」と読める答えですが、再試行で直る状態です)。これらは
