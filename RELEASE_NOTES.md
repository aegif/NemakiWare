...
- **webhook 受信 (`/core/api/v1/ingest-webhook/{connectorId}`) の受信先解決が索引を読まなく
  ...
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  ...
  - **コネクタ自身の解決も同じ扱い**です。コネクタの読み取りが答えられなかったとき
    ...
    **503 と 401 の違いから未認証で分かること**を、答えが実際に分ける組で書きます。
    503 は「その id に受信側が拒否する行 (読めない行 — 確定的 ID でも旧 ID でも —、または
    2 行) が在る」「ID 直読みが失敗した」「セレクタが失敗していて確定的 ID の行が無い」の
    いずれかで、どれかは分かりません。401 は「その id は不在」か「読める行が 1 つ在り、
    署名が合わなかった」のどちらかで、平常時は区別できません。**セレクタが失敗している
    間だけ**、401 (非 503) は「読める確定的 ID の行が在る」を意味します (不在はその間
    503 になるため)。この最後の開示は、セレクタが失敗している間は一律 503 にすれば消せ
    ますが、その間すべての webhook を止めることになるので採りませんでした。署名が合わない
    要求にその間だけ 401 でなく 503 を返せば、正しい送信側を止めずに消せます — 未対応の
    候補として記録しています。
...
- **コネクタ・取込プロファイルのセレクタ一覧に、200 件の無言の上限がありました。**
  ...
  (通信失敗のほか、この段で出るあらゆる実行時例外 — 原因は WARN ログに残ります) は
  拒否します。この拒否は、一覧を返す端点と一覧を直接使う経路 (コネクタ・取込プロファイル・
  フォルダ別コネクタの管理コントローラ) と、書き込み本体の既存行確認 (`POST` / `PUT` が
  保存前に同じ ID の行を探す段) では **503** です (従来は、作成・更新が個別に捕まえていた
  型付きの拒否を除き、いずれも 500 でした — 通信失敗は作成・更新でも 500 でした)。
