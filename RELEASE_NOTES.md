...
### 既知の制限: インスタンスが残っているタイプも削除できてしまいます

CMIS では、あるタイプのオブジェクトがまだ存在する間、そのタイプ定義の削除は
拒否されるべきですが、**現在は成功します**。上記の調査で判明した従来からの制限で、
今回の修正では変えていません (削除が拒否されるようになる挙動変更のため、別途対応します)。

タイプを削除する前に、そのタイプのオブジェクトが残っていないかは**運用側で確認**
してください。
...
> **追記 (2026-08-12): 26,416 文書で再実測しました。** 再索引中のピークは 5 で、
> アイドル時のピークも 5 — つまり再索引の上乗せは実質ゼロです。完了後も張り付かず、
> 20,000 件の作成・全再索引・全テストを含む 2 時間 15 分で接続漏れの警告は 0 件でした。
> **数十万文書の規模はまだ未実測**なので、その規模で運用される場合は再索引時の
> 接続数監視をお願いします。
