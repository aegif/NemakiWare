...
- **委譲取込の「書き込み直前の再認可」が、拒否として返るようになりました**
  (`POST /core/api/v1/repo/{repositoryId}/ingest`、**クライアント影響あり**)。取込は
  取得の前に認可ゲートを通りますが、取得の最中に権限が変わりうるため、書き込み直前に
  もう一度同じことを訊きます。この 2 回目の拒否だけが、**ゲートと違う状態コード**で
  返っていました。
  - リポジトリの取り違え (認証したリポジトリと違う) — 従来 **500** → **403**
  ...
  いずれもゲートが同じ状態に返している番号 (403 / 503) に揃えたものです。**拒否されたか
  どうかは変わりません** — 変わるのは番号だけです。500 で受けて「サーバ側の不具合」と
  扱っていた監視がある場合は、403 / 503 を見るように変えてください
