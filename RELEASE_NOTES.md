...
- **webhook 受信 (`/core/api/v1/ingest-webhook/{connectorId}`) の受信先解決が索引を読まなく
  ...
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  ...
  - **コネクタ自身の解決も同じ扱い**です。コネクタの行が在るのに読めない、または在るか
    どうかを確かめられないときは **503** です (従来は「署名不正」の **401**、または 500。
    401 を受けた送信側は再試行しません)。存在しないコネクタは従来どおり 401 です。
    Dropbox の URL 検証 (`GET`) も同じで、読めない行は 404 ではなく 503 になります
  - **受信先のプロファイル行が解釈できないときも 503** です。従来は解釈できない行を
    ...
    **受信中のコネクタを名指ししている行があれば**そのコネクタへの配送を拒否します
    (別のコネクタを名指す壊れた行は、その配送を止めません)。行が直る (またはその値を
    読めるノードに切り替わる) まで続く状態なので、ERROR ログに行の ID が出ます
...
- **コネクタ・取込プロファイルのセレクタ一覧に、200 件の無言の上限がありました。**
  ...
  続きを読めないページ・同じ bookmark の再来・`docs` の無い応答は **503** で拒否します
  (従来この形の失敗は 400 や 500 になり得ました)。索引不要にする変更ではありません —
