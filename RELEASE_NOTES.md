...
一覧では従来どおり WARN ログに行の ID を出して飛ばし、残りを 200 で返します。1 行のために
一覧全体を 503 にすると、その行を消すための一覧が見えなくなるためで、この版では変えて
いません。行単位で「読めない」を答えるのは、webhook の受信先解決と取込内の解決だけです。)
  ...
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  ...
  - **受信先のプロファイル行が解釈できないときも 503** です。従来は解釈できない行を
    ...
    **受信中のコネクタを名指ししている行があれば**そのコネクタへの配送を拒否します —
    ...
    壊れた行のその 2 欄自体が読める形でない場合は宛先を確かめられないので、**すべての
...
- **コネクタ・取込プロファイルのセレクタ一覧に、200 件の無言の上限がありました。**
  ...
  同じ一覧を内部で使う経路 (webhook の受信先、`/summary` / `/by-principal` /
  `/by-group`、archetype 別の一覧、書き込み前の既存行の確認) は、**201 件目以降を黙って
