...
- **webhook 受信 (`/core/api/v1/ingest-webhook/{connectorId}`) の受信先解決が索引を読まなく
  ...
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  ...
  - **受信先のプロファイル行が解釈できないときも 503** です。従来は解釈できない行を
    ...
    **逆に、`enabled` の文字列で無効と読むのは mapper が読む形だけになりました** —
    `"false"` / `"False"` / `"FALSE"` の 3 綴り (前後の空白は除かれます)、空文字や空白だけの
    文字列、および `"null"` (いずれも「値なし」として false)。`"fAlSe"` のようにそれ以外の綴りの行は、従来
    ...
    コネクタは止めません。
    `enabled` を false と読める壊れた行 (上に挙げた形 — リテラル、3 綴りの文字列、明示的な
    null、空文字や空白のみ、`"null"`) は受信先になり得ないので数えません。行が直る (またはその値を読めるノードに切り替わる) まで続く状態なので、
