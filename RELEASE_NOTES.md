...
- **webhook 受信 (`/core/api/v1/ingest-webhook/{connectorId}`) の受信先解決が索引を読まなく
  ...
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  ...
  - **コネクタ自身の解決も同じ扱い**です。コネクタの読み取りが答えられなかったとき
    ...
    見せたときも 503 です (索引の並び順で秘密や有効状態が決まる側には倒しません)。
    **このコネクタ解決の答え (503 か否か) だけから未認証で分かること**を、答えが実際に
    ...
    「読める行が 1 つ在る」のどちらかで、その後の答え (POST は無効な行でも署名不一致でも
    401、GET は無効・Dropbox 以外・challenge 不備で 404) はこの解決だけでは不在と区別
    しません。**セレクタが失敗している間だけ**、非 503 は「読める確定的 ID の行が在る」を
    意味します (不在はその間 503 になるため)。この最後の開示は、セレクタが失敗している間は
    一律 503 にすれば消せますが、その間すべての webhook を止めることになるので採りません
    でした。署名が合わない要求にその間だけ 401 でなく 503 を返せば、正しい送信側を止めずに
    消せます — 未対応の候補として記録しています。なお、受信側のプロトコル上のハンド
...
- **コネクタ・取込プロファイルのセレクタ一覧に、200 件の無言の上限がありました。**
  ...
  (通信失敗のほか、この段で出る実行時例外 — 一覧側が自分で出す上の 3 つの拒否を除き、
  原因は WARN ログに残ります) は拒否します。この拒否は、一覧を返す端点と一覧を直接使う
