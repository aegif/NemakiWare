...
- **「このノードに配線が無い」を「そんなものは無い」と答えていた読みを、さらに 5 か所
  ...
  - **`targetFolderPath` の解決**: 権限拒否やストアの失敗が「パスが無い」と同じ `null` に
    なり、呼び出し元が「このプロファイルには targetFolderId も targetFolderPath も設定されて
    いない」と答えていました (設定されているから解決を試みたのに)。しかもその応答は DLQ へ
    保存する前に返るため、**取り込めなかった項目が何の記録も残さず消えて**いました
