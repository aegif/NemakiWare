...
いません — 飛ばした行の ID はその WARN にあり、`?docId=` で消せます。行単位で「読めない」を
答えるのは、webhook の受信先解決とコネクタ解決、取込内の解決、そして「リポジトリごとに既定
1 つ」の規則が読む 4 欄 (下記) です。)
  ...
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  ...
  - **受信先のプロファイル行が解釈できないときも 503** です。従来は解釈できない行を
    ...
    **受信中のコネクタを名指ししている行があれば**そのコネクタへの配送を拒否します —
    ...
    壊れた行の `allowedArchetypes` がそのコネクタの種類 (archetype) を明確に含まない場合は
    宛先ではないので止めません (読める行なら受信側の archetype 判定で外れる行です)。
    壊れた行のこれらの欄自体が読める形でない場合は宛先を確かめられないので、**すべての
    コネクタ**への配送を拒否します (壊れた行 1 つで全 webhook が止まる側に倒しています —
    その行宛てのイベントが「該当なし」として消える側より、直すまで止まる側を取りました)。
