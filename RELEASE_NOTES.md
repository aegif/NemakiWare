...
- **記録するのは digest だけで、本文は入れません。** 台帳は purge されないので、
  参加者やアドレスを入れると消せない場所に個人データを固定することになります。
  入るのは repositoryId / intentId / connectorId / sourceObjectId と
  適用済みメタデータの hash から作った 1 個のハッシュです
- **台帳に書けなくても取込は成功します。** その時点で本文も capture 行も既に
  永続化されているので、2 本目の記録が書けないことで取込を失敗させると
  **その記録が対象にしていた当のものが消えます**。代わりに応答に警告が入ります
- **台帳が届かない間、警告は取込ごとに呼び出し元へ返りますが、サーバログは
  最初の 1 件と以後 100 件ごと**にまとめます (件数つき)。全件出すと障害の開始を
  告げる行が同じ行 1 万本に埋もれるためです
