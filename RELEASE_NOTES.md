...
- **取込の拒否が 500 で返らなくなりました** (**クライアント影響あり**)。**拒否されるか
  どうかは変わりません — 変わるのは番号だけです。** 取込は取得の前に認可ゲートを通り、
  取得の最中に権限が変わりうるため書き込み直前にもう一度訊き、さらに書き込みそのものが
  CMIS の ACL 評価を通ります。この **2 番目と 3 番目**の拒否が、1 番目 (ゲート) と違う
  番号で返っていました。
  - 対象フォルダの `cmis:all` が書き込み時点で無い — 従来 **400** → **403**
    (400 だったのは文面に「is required」が含まれていた偶然によるもので、意図では
    ありません)
  - コネクタの委譲が取消済み — 従来 **500** → **403**
  - 書き込み自体が CMIS の権限で拒否された (再認可の直後に剥奪された場合を含む) —
    従来 **500** → **403**
  - 認可サービスがこのノードに配線されていない — 従来 **500** → **503**
    (「訊けなかった」であって「訊いて断られた」ではありません)
  - 取込側が**自分で「一時的」と判定した**失敗 (接続断・タイムアウト・429/502/503/504・
    レート制限。401/403/404 は明示的に除外) — 従来 **500** → **503**。従来はその判定を
    本文の `[transient]` 印にだけ残して、番号は捨てていました
  - メタデータが上限 (1 MB) 超過、またはメタデータが JSON として壊れている —
    従来 **500** → **400**。同じ入口が 1 つ上の層では「File exceeds maximum size (100MB)」
    に 400 を返しており、「要求が大きすぎる」に 2 つの答えがありました

  500 で受けて「サーバ側の不具合」と扱っていた監視がある場合は、403 / 503 / 400 を
  見るように変えてください
- **DLQ の再実行が、拒否を `200 "failed"` で返さなくなりました**
  (`POST /core/api/v1/admin/ingest/dlq/{id}/retry`、**クライアント影響あり**)。
  `200` + `status:"failed"` + `retryCount` は「もう一度試せ」という答えですが、認可の
  拒否は何度試しても同じです。**認可の拒否のときだけ 403** になり、本文に理由と
  「エントリは残した」ことが入ります。**それ以外の失敗は従来どおり 200** です
  (`{"status":"failed","retryCount":N}` を読むクライアントは影響を受けません)。

  この入口は**既定リポジトリに束縛**されており (`AuthenticationFilter` が `/v1/admin/*` を
  そう扱います)、再実行する要求は**保存時のリポジトリ**を持っています。書き込み直前の
  再認可はリポジトリの confinement を管理者の特例より先に見るため、**既定以外の
  リポジトリの委譲エントリの再実行は毎回拒否されます**。これは 3.4 で新しく閉じた
  越境の一部です。従来この拒否は `200 "failed"` に見えていました
