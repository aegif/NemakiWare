...
- **webhook 受信 (`/core/api/v1/ingest-webhook/{connectorId}`) の受信先解決が索引を読まなく
  ...
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  ...
  - **コネクタ自身の解決も同じ扱い**です。コネクタの読み取りが答えられなかったとき
    ...
    書いた行 (次の起動まで報告されません) です。なお、確定的 ID の行が在るのに読めない
    (壊れている) 場合も 503 なので、**壊れた行が在ることだけは 503 と 401 の違いから
    分かります** (健全な行の存在は分かりません)。
...
- **コネクタ・取込プロファイルのセレクタ一覧に、200 件の無言の上限がありました。**
  ...
  続きを読めないページ・同じ bookmark の再来・`docs` の無い応答は拒否します。この拒否は、
  一覧を返す端点と一覧を直接使う経路 (コネクタ・取込プロファイル・フォルダ別コネクタの
  管理コントローラ) では **503** です (作成・更新のように個別に捕まえていた端点以外は、
  従来は 500 でした。作成の既存行確認では 400 でした)。1 件の読み取り (`GET` by id、
  `PUT` の既存行解決) はセレクタが答えなければ確定的 ID の直読みにフォールバックするので、
