...
- **webhook 受信 (`/core/api/v1/ingest-webhook/{connectorId}`) の受信先解決が索引を読まなく
  なりました。** 従来は Mango セレクタで取込プロファイルを一覧していたため、索引再構築中は
  「該当プロファイルなし」(`no_profile`、**200**) を返し、送信側はイベントを配送済みとして
  扱っていました。一覧が完了できないときは **503** を返します (送信側が再試行してよい
  唯一の答えです。従来は例外なら 500 でした)。`repositoryId` を持たない行は受信先に
  なりません — 委譲ゲートがどのリポジトリに対しても拒否する行で、配送しても失敗するだけ
  でした
- **コネクタ・取込プロファイルのセレクタ一覧に、200 件の無言の上限がありました。**
  `GET /core/api/v1/admin/connectors` と `GET /core/api/v1/admin/import-profiles`、および
  その一覧を絞り込む内部経路は、**201 件目以降を黙って落として**いました (索引再構築とは
  無関係に、毎回)。bookmark で続きを読むようになり、続きを読めないページは拒否します。
  索引不要にする変更ではありません — 再構築中に索引が見せない行は、これまでどおり
  見えません
- **リンク作成の重複検査が答えられなかったとき、黙らなくなりました。** 既存の関係を
  読めなかった場合、リンクは従来どおり作ります (一過性の読み失敗で正当な初回リンクを
  止めない) が、結果の `warnings` に「重複検査なしで作った — 重複エッジが出来た可能性が
  ある」を返し、capture 記録にも同じ事実を載せます。従来は検査の失敗を「関係は無い」と
  同じ値にして、何も残していませんでした。取込完了後に orchestrator が張るリンク
  (Slack / Teams / Mattermost) では、この警告は取得サマリの `errors` に入ります —
  そこには他の経路がありません
- **「リポジトリごとに既定プロファイルは 1 つ」の検査が、規則が読む 4 欄 (`profileId` /
  ...
  新しいノードが書いた値、または破損) が 1 行あるだけで、そのリポジトリの取込プロファイルの
  作成 (**400**) と更新 (**503**) が全部止まっていました。その行は今も規則に数えられます
  (既定が 2 つになる経路は開いていません)。規則の 4 欄そのものが読めない行は従来どおり
  拒否します
