...
## コネクタ定義の保存 ID を確定的にし、旧 ID の行は起動時に移行します

外部取込のコネクタ定義は `connector_definition:<connectorId>` という**確定的な ID** で
保存されるようになりました。既存環境に残っている**生成 ID の旧行は、起動のたびに走る
移行が自動で書き直します** (書き直し済みなら何もしません)。

- **移行は索引を読みません** (`_all_docs` と ID 直読みだけ) — Mango 索引の再構築中、
  つまり重複が生まれ得た唯一の窓でも、読みが欠けない構造です
- **コネクタの新規作成そのものにも索引不要の重複検査が入りました。** セレクタと確定的
  ID の両方をすり抜ける旧行が残っていても、作成側が `_all_docs` で見つけて拒否します
  (移行の成否に依存しません)
- **同じ connectorId の定義が 2 つあり、内容が食い違う場合、移行はどちらも触りません。**
  起動のたびに ERROR ログで両方の ID を報告します。**要らない方の行だけ**を
  `DELETE /core/api/v1/admin/connectors/{connectorId}?docId=<行の ID>` で削除して
  ください (docId 無しの DELETE は該当 connectorId の**全行**を消します)。自動で
  どちらかを選ぶことはしません — それは設定の静かな喪失です
- 移行が失敗した行は次の起動で再試行します。添付ファイルを持つ旧行は**移行しません**
  (不完全な複製を作るより、報告して残します)

### コネクタ管理 API の応答が 3 か所変わりました
