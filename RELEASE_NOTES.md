...
- **webhook 受信 (`/core/api/v1/ingest-webhook/{connectorId}`) の受信先解決が索引を読まなく
  ...
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  ...
  - **コネクタ自身の解決も同じ扱い**です。コネクタの読み取りが答えられなかったとき
    ...
    **503 と 401 の違いから未認証で分かること**は 2 つあります: 確定的 ID の行が在って
    読めない (壊れている) こと (常に)、そして**セレクタが失敗している間だけ**、確定的 ID の
    行が在るかどうか (読める行は答え、行が無ければ旧 ID 行を除外できないので拒否するため)。
    後者は索引障害の窓に限ります — セレクタが失敗している間は一律 503 にすれば消せますが、
    その間すべての webhook を止めることになるので、窓の間だけ存在が分かる側を取りました。
...
- **コネクタ・取込プロファイルのセレクタ一覧に、200 件の無言の上限がありました。**
  ...
  続きを読めないページ・同じ bookmark の再来・`docs` の無い応答・セレクタの通信失敗は
  拒否します。この拒否は、一覧を返す端点と一覧を直接使う経路 (コネクタ・取込プロファイル・
  フォルダ別コネクタの管理コントローラ) と、書き込み本体の既存行確認 (`POST` / `PUT` が
  保存前に同じ ID の行を探す段) では **503** です (作成・更新のように個別に捕まえていた
  端点以外は、従来は 500 でした)。1 件の読み取り (`GET` by id と、`PUT` / `DELETE` の
  入口で定義を解決する段) はセレクタが答えなければ確定的 ID の直読みにフォールバックする
  ので、そこでは一覧の不完全を理由にした 503 は出ません。索引不要にする変更ではありません —
  索引が見せない行は、これまでどおり見えません
