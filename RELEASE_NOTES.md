...
- **取込プロファイル (`/core/api/v1/admin/import-profiles`) も同じ扱いです。**
  ...
  **503** を返します (存在しないプロファイルは従来どおり 404)。コネクタの `GET` も同じです。
  なお確定的 ID の行も、旧 ID しか無い行も、索引を使わずに読めるので多くの場合そもそも 200 が
  返ります。503 になるのは、行を読もうとして読めなかった場合です。同じ ID の行がその
  リポジトリに 2 つある場合、`PUT` / `POST` / 所有権移転と `DELETE` (docId 無し) は **409**
  で、先にどちらかを `?docId=` で片付ける必要があります。**`GET` はこの状態でも 200 を返します**
