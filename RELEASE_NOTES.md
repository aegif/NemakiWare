...
- **webhook 受信 (`/core/api/v1/ingest-webhook/{connectorId}`) の受信先解決が索引を読まなく
  ...
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  ...
  - **コネクタ自身の解決も同じ扱い**です。コネクタの読み取りが答えられなかったとき
    ...
    **この対の拒否はセレクタが見せた対だけ**です。セレクタが失敗している間に確定的 ID の
    行が読めれば、それが答えになります — 旧 ID の相方は除外できないので、その相方が別の
    secret や無効状態を持っていても受信は確定的 ID の行の設定で走ります。一律に拒否すれば
    塞げますが、**確定的 ID による読みはこの batch が索引不要の経路として用意したもの**で、
    セレクタが例外を返すのは使える Mango 索引が無いとき (新しいノード、作り直した DB) —
    つまり §62 の相方が生まれる窓そのものです。そこで拒否すると、通し続けるために作った
    経路が、必要なちょうどその窓で止まります。**なおこの窓の長さは測っていません**
    (以前の版はここで v3.3.0 の runbook の「時間単位」を引きましたが、あれは Solr/CMIS の
    再索引の測定で、設定 DB の Mango 索引については何も言っていません。判断はその数字には
    依っていません)。相方の secret で署名されたイベントは
    やはり検証に落ちること、その間の管理 API の書き込みは索引を使わない行数え (対を見ます)
    で拒否されること、この相方は移行が回収しきれていない upgrade 済み環境にしか無いことで
    範囲を限っています。
