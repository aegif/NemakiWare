...
`import_profile_definition:<profileId>` という**確定的な ID** で保存されるようになりました。
既存環境に残っている**生成 ID の旧行 (既定の `cloud-import-<repo>` プロファイルを含む) は、
起動のたびに走る移行が自動で書き直します** (書き直し済みなら何もしません)。

- **移行は索引を読みません** (`_all_docs` と ID 直読みだけ) — 重複が生まれる典型的な窓
  である Mango 索引の再構築中でも、索引の状態に左右されずに走ります。ただし走査は
  スナップショットではないので、走査中に別のノードが書いた行は次回に回ります
- **コネクタの新規作成そのものにも索引不要の重複検査が入りました。** セレクタと確定的
  ID の両方をすり抜ける旧行が残っていても、作成側が `_all_docs` で見つけて拒否します
  (移行の成否に依存しません)
- **同じ ID (connectorId / profileId) の定義行が 2 つ以上あり、内容が食い違う場合、
  移行はどれも触りません** (旧 ID の行が 2 つ並んでいる場合も同じです)。内容が完全に同一の
  残骸 (旧 ID の行 1 つ + 確定的 ID の行 1 つ) だけは、旧 ID の行を回収します。
  食い違う行が残っている間は、起動のたびに ERROR ログで両方の ID を報告します。
  **要らない方の行だけ**を
  `DELETE /core/api/v1/admin/connectors/{connectorId}?docId=<行の ID>` (プロファイルは
  `DELETE /core/api/v1/admin/import-profiles/{profileId}?docId=<行の ID>`) で削除して
  ください (docId 無しの DELETE は、コネクタでは該当 ID の**全行**を、プロファイルでは
  **呼び出し元リポジトリの行だけ**を消します。同じ profileId の行が別のリポジトリに残って
  いる場合、その行はそのリポジトリの管理者が `DELETE` (docId 無し) で消すまで残り、`PUT` は
  409 を返し続けます。同じ profileId が 2 つのリポジトリにある間、`GET` と `DELETE` は
  **呼び出し元リポジトリの行**に対して働きます — どちらの行が索引から先に返るかには依存
  しません。`PUT` と所有権移転は、書き込み側が profileId の行をリポジトリ横断で数えるため 409 の
  ままです — 数えているのは確定的 ID ではなく `type` + `profileId` です)。
  自動でどちらかを選ぶことはしません — それは設定の静かな喪失です
- **`?docId=` は「最後の 1 行」を消せません (409)。** この操作は食い違う 2 行を片付けるための
  もので、定義が残る前提でスケジューラ停止も「プロファイルを削除した」という扱いもしません
  (**プロファイル側**は行を消した監査記録が残ります。コネクタ側の管理 API には
  この監査がありません)。最後の行を消すと定義
  ごと消えてしまうため拒否します。定義そのものを消すときは `docId` 無しの `DELETE` を
  使ってください
- 移行が失敗した行は次の起動で再試行します。添付ファイルを持つ旧行は**移行しません**
  (不完全な複製を作るより、報告して残します)。**`repositoryId` を持たないプロファイル行も
  移行しません** — 正規の作成経路では作れない壊れた行で、確定的 ID に書き直すと
  リポジトリ限定の削除 API から二度と届かなくなるためです。起動のたびに ERROR で報告されます。
  **この行は `?docId=` を付けた `DELETE` で消せます** (どのリポジトリの管理者からでも — 行が
  どのリポジトリにも属さないので、誰かのプロファイルが消えることはありません)。放置すると
  その profileId の `PUT` が恒久的に 409 になります

### コネクタと取込プロファイルの管理 API で、応答が変わりました

- **`POST /core/api/v1/admin/connectors` は、`credentialRef` / `webhookSecret` に文字列
  `"[configured]"` を渡すと 400 を返します。** これは GET が秘密の代わりに返すマスクで、
  新規作成には復元元がありません。従来は文字どおり保存され、認証できないコネクタが
  201 で出来ていました
- **`PUT` で、保存済みの行が索引からもまだ見えない瞬間 (索引再構築中) は 503 を
  返します。** 従来は 500、または**見えない既存行の隣に新しい行を書いてしまう 200**
  でした。503 は再試行すれば通る合図です。この窓の間は実値を載せた PUT も 503 に
  なります — 見えていない行の上書きや分裂を避けるためで、索引が追いつけば同じ
  PUT がそのまま通ります。索引が「索引不要の走査より多くの行」を報告した場合も 503 です
  (2 つの読みが食い違っている間は、どの行に書くかが決まりません)
- **取込の自動解決で、条件に合うコネクタが 2 つ以上あるときはエラーになります。** 従来は
  どれが返るか決まっておらず、そのうちの 1 つが黙って選ばれていました。リクエストで
  `connectorId` を指定するか、不要な方を無効化してください。**`google` と `google_drive`
  のように別名で 1 件ずつ一致する場合は従来どおり**、リクエストが使った綴りの方が選ばれます
  (別名の順序は優先順位であって曖昧さではありません)
- **`PUT` で、同じ ID の定義行が 2 行以上ある場合は 409 を返します** (索引がその行を
  見せているかどうかに関わらず。行数は索引を使わずに数えます)。索引再構築中に出来た
  重複行が、起動時移行で自動解決されなかった (内容が食い違う) ときの状態です。従来は
  先頭の行に書いて 200 を返し、もう一方が黙って上書きされていました。503 ではありません —
  再試行では直らず、`?docId=` で不要な行を削除してから PUT してください。取込
  プロファイルの `PUT` も同じです
- **削除が途中で失敗した場合は 503 を返します。** 定義行は 1 行ずつ削除され、CouchDB に
  文書をまたぐトランザクションはありません。途中で失敗すると一部の行は既に消えているので、
  「何も起きなかった」と読める 500 ではなく再試行の合図を返します。このとき、プロファイル側は
  監査ログにも失敗として記録します。**1 行も消せずに失敗した場合は 503 になりません** —
  それは保存側が操作ごと拒否している状態で、部分削除としては扱いません
- **取込プロファイル (`/core/api/v1/admin/import-profiles`) も同じ扱いです。**
  索引再構築中に見えない**プロファイル**への `PUT` / `DELETE` は、従来の **404** では
  なく **503** を返します (存在しないプロファイルは従来どおり 404)。**コネクタ側は元から
  違います** — `PUT` は行が無ければ作りますし (200)、`DELETE` は消す行が無くても 200 です。
  コネクタで 404 を返すのは `GET` だけで、そこに 503 との分割が入りました。**プロファイルの `GET` は、索引が
  答えられなければ確定的 ID の行を索引なしで読み直します** (索引をまず引き、答えられなかった
  ときに読み直す順です) — 確定的 ID の行も、旧 ID しか無い行も、そうして読めるので多くの
  場合 200 です。`GET` が 503 になるのは、その読み直しも答えられなかった場合で、行が在るか
  どうかまで分かっているとは限りません。**コネクタの `GET` は少し違います** — 索引を使わない
  読みで確かめるのは「行が在るかどうか」だけで、行そのものは取り直しません。旧 ID しか
  無いコネクタは、索引が答えられる間はセレクタが拾って 200 ですが、**索引が答えられない間は
  404 ではなく 503** になります (起動時の移行が確定的 ID に書き直すまでの間)。
  同じ ID の行がそのリポジトリに 2 つある場合、`PUT` / 所有権移転と `DELETE` (docId 無し) は
  **409** で、先にどちらかを `?docId=` で片付ける必要があります (`POST` はこの状態でも
  **400** です — 下の箇条書きと同じで、409 ではありません)。**`GET` はこの状態でも 200 を
  返します** — どちらの行かは索引の返す順に依存します (書ける動詞が全て止まるので、読みだけを
  止める意味が薄いのと、管理 API 全部を索引不要にすると設定 DB の全走査が毎回入るため)。
  **索引が答えられないときの `GET` も、確定的 ID に呼び出し元の行があればそれを返して 200**
  です (この読みは索引を使いません)。**409 / 503 になるのは、確定的 ID に呼び出し元の行が
  無いとき**です。プロファイルでは 2 行とも旧 ID である場合や確定的 ID の行が別のリポジトリ
  のものである場合で、**409**。コネクタはリポジトリで分かれないので 409 の経路が無く、
  読み直せなければ **503** です。
  他のリポジトリが同じ ID を持っているだけの場合は、`?docId=` ではなく**そのリポジトリの
  管理者による `DELETE`** が要ります。どのリポジトリにも属さない行 (移行が残すことがあります)
  だけは `?docId=` で消せます — この行は全域カウントに数えられるため、放置すると
  その profileId の `PUT` が恒久的に 409 になります。**この行は取込にも使えません** —
  以前は「どのリポジトリにも属さない」が「どのリポジトリからでも使える」として通っていました
- 旧 ID の行が残っているプロファイルを `POST` で作り直そうとすると 400、索引再構築中の
  `PUT` は 503 です。「リポジトリごとに既定プロファイルは 1 つ」の検査も索引を読まなく
  なり、**索引再構築を理由に** 2 つ目の既定が通ることはありません
- **ファイルシステム export は、上書き先の既存ファイルを壊さなくなりました。** コピーが
  完走したときだけ置き換えます (途中で失敗しても前回の export はそのまま)。また
  export したファイルの権限は従来どおり umask 由来になります (一時期の実装で 0600 に
  落ちる回帰がありました。リリース前に修正済みです)
- **「読めなかった」が 500 や 400 で返っていた入口を 503 に揃えました。**
  DLQ の再実行 (`POST /core/api/v1/admin/ingest/dlq/{id}/retry`)、取込
  (`POST /core/api/v1/repo/{repositoryId}/ingest`、JSON と multipart の両方)、
  スケジューラの一覧・手動起動 (`GET /core/api/v1/admin/ingest-scheduler/status`、
  `POST .../trigger/{profileId}`)、webhook の購読管理
  (`POST` / `DELETE /core/api/v1/ingest-webhook/{connectorId}/subscribe`) は、
  行が読めなかった場合に Spring の **500** を返していました (確定的 ID に別の文書が居る場合の
  ほか、スケジューラの一覧では走査そのものが完了できない場合、既定コネクタの解決が拒否を
  返す場合を含みます) (「こちらの不具合」と読める答えですが、再試行で直る状態です)。これらは
  **503** になります。**DLQ の再実行と購読管理は、索引再構築中に行が見えないだけの場合は
  対象外**です — その場合コネクタの読みは従来どおり null を返し、購読管理は **404** の
  ままです (この呼び出し元は設計台帳に残件として記録されています)。取込だけは下記のとおり
  この場合も 503 です。webhook 側の本文は理由を含みません — この
  コントローラは未認証の受信口を同居させており、理由の文面はその id に行が在るかどうかを
  明かすためです (購読管理は管理者専用なので、理由はログにだけ残ります)
- **委譲取込は、取得の最中に権限が変わると書き込みを拒否するようになりました**
  (`POST /core/api/v1/repo/{repositoryId}/ingest`、**クライアント影響あり**)。3.3 では
  認可はゲート (取得の前) で 1 回だけでした。長い取得の最中に対象フォルダの `cmis:all`
  が剥奪されても、あるいはコネクタの委譲が取り消されても、**書き込みは完走していました**。
  3.4 は書き込みの直前にもう一度同じことを訊き、変わっていれば **403** で拒否します。
  - 対象フォルダの `cmis:all` が書き込み時点で無い → **403** (3.3 では書き込み成功)
  - コネクタの委譲が取消済み → **403** (3.3 では書き込み成功)
  - 認可サービスがこのノードに配線されていない → **503**
    (「訊けなかった」であって「訊いて断られた」ではありません)

  **原子的ではありません。** ここと実際の書き込みの間に入った剥奪は捕まえられません
  (その場合は下記の CMIS 権限拒否として 403 になります)
- **取込の拒否・一時障害・要求の誤りが 500 で返らなくなりました**
  (**クライアント影響あり**)。以下は 3.3 でも同じ条件が起き、いずれも **500** でした。
  - 書き込み自体が CMIS の権限で拒否された → **403**。ゲートは 3.3 から 403 を返して
    おり、3 番目のこの関門だけが 500 のままでした (2 番目の再認可は 3.4 で新設したもので、
    上の項のとおりです)
  - 取込側が**自分で「一時的」と判定した**失敗 (接続断・タイムアウト・429/502/503/504・
    レート制限。401/403/404 は明示的に除外) → **503**。判定を本文の `[transient]` 印に
    だけ残して番号は捨てていました。mail / note / business record / chat の 4 経路は
    印すら本文に載っていませんでした
  - メタデータが上限 (1 MB) 超過、またはメタデータが JSON として壊れている → **400**。
    同じ入口が 1 つ上の層では「File exceeds maximum size (100MB)」に 400 を返しており、
    「要求が大きすぎる」に 2 つの答えがありました

  状態コードは**本文の文面で決まります**。拒否の文面は製品が組み立てるものと照合して
  いますが、文面にリポジトリ ID・オブジェクト ID・オブジェクト名が差し込まれる以上、
  完全な分類ではありません (例: 名前に `503` を含むフォルダが、拒否を一時障害に
  見せかけないよう、拒否の判定を先に置いています)。500 で受けて「サーバ側の不具合」と
  扱っていた監視がある場合は、403 / 503 / 400 を見るように変えてください
- **設定 DB が答えないときに、取込が「記録は無い」「未取得」と断定しなくなりました**
  (**データ保全**)。`nemaki_conf` の読みが失敗すると、空の設定が `loadFailed` 付きで
  返り、値は `null` になります。この `null` を 2 か所が事実として述べていました。
  - **冪等性レコード**: 「この要求はまだ完了していない」と読み、`dedupePolicy=replace`
    なら**前回の実行が作った文書を削除して作り直して**いました → **503** で拒否します
  - **コネクタの checkpoint**: 「このプロファイルは一度も取得していない」と読み、
    先頭ページだけを取って全件を新規扱いし、**取得しなかった古い項目を飛び越えて**
    checkpoint を進めていました (以後その項目は恒久的に対象外) → tick を失敗させます

  値がシステムプロパティ・環境変数・properties ファイルで解決できる場合は従来どおりで、
  拒否は**どこからも解決できず、かつ設定 DB が答えなかった**ときだけです
- **DLQ の再実行と一覧・purge が、答えられなかったことを黙って埋めなくなりました**
  (`/core/api/v1/admin/ingest/dlq/...`、**クライアント影響あり**)。
  - **再実行が恒久的に拒否されたとき 403** になります (`200` + `status:"failed"` +
    `retryCount` は「もう一度試せ」という答えですが、拒否は何度試しても同じです)。
    本文に理由と「エントリは残した」ことが入ります。**それ以外の失敗は従来どおり 200**
    (`{"status":"failed","retryCount":N}` を読むクライアントは影響を受けません)。
    403 になるのは、状態コードの分類が **FORBIDDEN** になる拒否です — 委譲の再認可の
    ほか、**プロファイルの許可リストに合わなくなったコネクタ / archetype** も含みます
    (これも再実行では直りません)
  - **`retryCount` が 1 つ多く返っていました。** 予約が行を N+1 に更新した後、応答が
    さらに 1 を足していました。保存された値と一致します
  - **予約が「試せなかった」ときに 503** になります。従来は書込競合に負けた場合と同じ
    `429`「他の再実行が進行中です」で、進行中でないものを進行中と述べていました
  - **中断した purge が 503** になります。従来は読みが失敗しても
    `{"status":"success","deleted":0}` を返しており、「期限より古いものは無かった」と
    区別が付きませんでした。実際に消えた件数は拒否の本文に入ります。1 行の削除が
    **競合以外**で失敗した場合も同じです (従来は競合と同じ「行が動いた」として数えず
    先へ進み、`success` で返っていました)。**なお 1 回の purge は先頭 1,000 行しか
    走査しません** (順序は保証されません。このブランチ以前からの上限です)。DLQ に
    それより多くの行がある場合、期限より古い行が走査されずに残ることがあり、**応答は
    それを示しません** (`deleted: 0` でも走査外の行は残りえます)。残りを消すには一覧を
    `?offset=` で辿って古い行を見つけ、`DELETE /dlq/{dlqId}` で個別に消してください
    (こちらも **store が削除を確認した行だけ**を数えます。確認できない行が残れば
    `success` とは答えません — `DELETE` は **503**、再実行の後片付けは `*-entry-kept` です)
  - **一覧が、読めない行のせいで「ここで終わり」と言わなくなりました。**
    `hasMore` は store が返した行数で判断します。読めない行があるときは
    `unreadableEntries` が付き、`count` がページの全部でないことを明示します
  - **bytes が保存されなかった試行を記述する行の再実行が 409 になります。** 暗号鍵が
    無い、あるいは添付の書き込みが通らなかった行は、再実行が**中身の無い文書を作って
    「成功」とし、その行を削除**していました。**行が過去の試行の添付を持っている場合も
    同じく拒否します** — その添付は古い試行のもので、行が記述しているのは新しい試行なので、
    再実行すると**古い bytes と新しいメタデータを組み合わせた文書**が「復旧した項目」に
    なります。いずれもコネクタから取り直してください。**保存時に既存の行が読めなかった行も
    同じ扱い**です (その行が持つ payload を今の試行に帰属させられないため、409 で拒否します)。

    **届いた payload が拒否される場合があります**: 添付を書いたあとの「確定書き込み」は、
    書き込み前に読んだ版に対して条件付けます。その読みが答えられなかったときに使う
    予備の腕は、添付の書き込みで版が上がった後の行に古い版で条件付けるため**必ず負けます**。
    結果その行は「確定していない」状態に留まり、**実際には payload が届いていても
    再実行は 409 になります**。コネクタから取り直してください。安全側 (無い payload を
    在ると言わない) に倒れる形ですが、過剰な拒否です

  - **確定書き込みの「この行は自分のものか」という検査の範囲**: 検査は行が持つ
    書き込みトークンの一致で行います。中身を伴わない保存は既存のトークンを引き継ぐため、
    **同じ項目について中身なしの保存が並行したとき、後の行を自分の行と見なすことがあります**。
    結果は 3.4 以前と同じ組み合わせで退行ではありませんが、この検査が保証するのは
    「**payload を伴う別の保存に上書きされない**」ところまでです
  - **一覧の `nextOffset` は次のページがあるときだけ付きます。** また、**一覧は
    `(type, dlqId, _id)` 順に並ぶようになりました。** 従来は並び順を指定しておらず、
    Mango が選んだ索引の順でした — CouchDB 3.3.3 の実測では `_id` 順でしたが、
    **どこにも保証されていない順序**だったため、`offset` は「未規定の並びの上の境界」で、
    前のページに出た行がもう一度出たり、どのページにも出ない行が生じたりしえました。
    `_id` まで並べるのは、**同じ `dlqId` の行が 2 つ在りうる**ためです (確定 `_id` に
    する前の形で書かれた双子行。単独削除の応答が以前から扱っています)。この起動から
    `(type, dlqId, _id)` の Mango 索引を登録します。**ページを見ている間にエントリを
    再実行・削除するとずれる**点は変わりません (その場合は一覧を取り直してください)
  - **並べられなかったページは、そうと分かるように返します。** 索引がまだ登録されて
    いないデータベースでは、一覧は従来どおりの並びで返りますが、応答に
    `"stableOrder": false` と説明が付きます (一覧そのものを拒否はしません — 各行は
    その項目が失われたことの唯一の記録だからです)
  - **store が文書一覧を返さなかった読みを「行が無い」と答えなくなりました。** 一覧は
    空ではなく **503**、単独の削除 (`DELETE /dlq/{dlqId}`) は 404 ではなく **503**、再実行の
    予約と purge はそれぞれの拒否になります (ジョブ一覧も同様)。設定 DB 側の同じ形は
    以前から拒否していましたが、DLQ とジョブの読みだけが空に潰していました

  なお再実行の入口は**既定リポジトリに束縛**されています
  (`AuthenticationFilter` は `/v1/admin/*` をそう扱います。ただし
  `/v1/admin/import-profiles/**` と `/v1/admin/connectors/**` だけは
  `X-Nemaki-Repository` を見ます)。再実行する要求は**保存時のリポジトリ**を持ち、
  書き込み直前の再認可はリポジトリの confinement を管理者の特例より先に見るため、
  **既定以外のリポジトリの委譲エントリの再実行は毎回 403 になります**。3.4 で新しく
  閉じた越境の一部で、従来この拒否は `200 "failed"` に見えていました
- **取込で、コネクタが読めなかったときに「コネクタ指定なし」として別の取込フローに
  振られることがなくなりました。** `connectorId` を指定した取込は、そのコネクタの
  archetype で処理フローが決まります。従来はコネクタの読みが失敗すると archetype が
  null になり、**ファイル名や `sourceObjectType` からの推測**に落ちていました
  (例: `sourceObjectType=message` を指定した CHAT_CONTEXT のコネクタが、メールとして
  解析・登録される)。フローの側は archetype を再確認しないため、間違った形のまま保存
  されます。今は、指定されたコネクタの archetype を確定できない場合は **503** を返します。
  **コネクタが存在しないと確かめられた場合は従来どおり**推測に落ちます (存在しない
  connectorId を 503 にはしません)。**コネクタの行は読めたが `sourceArchetype` が
  入っていない場合は 409** です — 再試行では直らず、その行を直す必要があります (作成・更新
  API はこの欠落を拒否するので、これは古い行か手で書いた行です)。なお、指定された
  connectorId をセレクタが解決できなかったときは、この判定のために設定 DB を 1 回走査します
  (委譲取込の経路は以前から同じ走査を通っています)。この走査は**設定 DB の中に読めない行が
  1 つでもあると拒否する**ので、無関係な壊れた行が 1 つあると、未登録の connectorId を
  指定した取込がすべて 503 になります (**行の本体が返らない・id が無い・列挙が進まない**
  場合です。本体は返ったが逆直列化できない行は、この走査では数えられないだけで拒否には
  なりません)。**委譲取込で読みが拒否された場合も監査に残ります**
  (以前はこの経路だけ記録が残りませんでした)
- **「このノードに配線が無い」を「そんなものは無い」と答えていた読みを、さらに 5 か所
  直しました。**
  - **DLQ の項目取得**: 保存されている行が読めない場合、**404「DLQ entry not found」**では
    なく **503** です。この行は失われた項目の唯一の記録なので、「無い」と答えるのが最も
    危険でした
  - **重複検査**: コンテンツストアが配線されていないノードで「既存の文書は無い」と答えて
    いたため、**重複を作って成功と報告**していました。今は拒否します
  - **`targetFolderPath` の解決**: 権限拒否やストアの失敗、フォルダでないものへの解決、
    ストアが何も返さなかった場合が、すべて「パスが無い」と同じ `null` になり、呼び出し元が
    「このプロファイルには targetFolderId も targetFolderPath も設定されていない」と
    答えていました (設定されているから解決を試みたのに)。4 例のうち 3 例で**状態コードも
    変わりました** — ストアの失敗、およびストアが何も返さなかった場合は **400 → 503**、
    **権限拒否は 400 → 403** (再試行では直らないため。一時期 503「retry shortly」でした)、
    フォルダでないものへの解決は 400 のまま (文面は変わります)、
    パスが存在しない場合も 400 のままです。**ただし取り込めなかった項目が記録を残さない点は
    変わっていません** — この応答は DLQ へ保存する前に返ります。DLQ に載せる修正は別途行います
  - **IMAP IDLE のコネクタ解決**: 配線が無いノードで「このプロファイルにはコネクタが無い」
    となり 400 でした。今は 503 です
  - **再取込の由来イベント**: コネクタが読めないと**イベントを黙って諦めて**いました
    (証跡は変わっているのに)。今は応答の `warnings` に「証跡は変わったが由来イベントは
    記録できなかった」と入ります
- **DLQ の再実行が、読めなかった中身を「中身なし」として扱わなくなりました。** 保存された
  payload が読めない場合 (暗号鍵のローテーション、復号できない本文、添付の読み取り失敗、
  100MB 超) に、従来は**中身なしで再実行**され、メタデータだけの文書が取り込まれて成功と
  記録され、**DLQ の行が削除**されていました。その行は失われた項目の唯一の記録です。今は
  **503** を返して行を残し、何も取り込みません (中身を持つと記録されているのに payload が
  返らない場合は **409**。ただし「中身を持つ」が保存時に store が答えられず**仮定**で記録
  された行は、store が「無い」と答えた時点で仮定が外れたものとして**中身なしで再実行**され、
  応答に `payloadPresenceAssumptionCleared: true` が付きます — 成功すれば行は削除されます)
- **DLQ の再実行について 2 点。** 再実行は**送出前に 60 秒のクールダウンを確保する**ので、
  上記の 503 を受けてすぐ再試行すると **429** が返ります (60 秒待ってください)。また、
  コネクタの行が**索引から見えないだけ**の場合、再実行は 503 ではなく **200** で
  `{"status":"failed", errors:[... could not be read; retry shortly]}` を返します — 再実行
  そのものは実行され、その中の取込が拒否された、という形です
- **IMAP IDLE の開始・停止 (`POST /core/api/v1/admin/ingest-scheduler/idle/start/{profileId}`
  ほか) の応答コードを分けました。** 従来はどんな拒否も **400** で、本文に「retry shortly」と
  書きながら 400 という状態でした。内訳は次のとおりです。
  - **503**: 読めなかった場合、およびこのノードに IDLE monitor / scheduler が配線されて
    いない場合 (別のノードで再試行してください)
  - **409**: 同じ ID の定義行が 2 つある場合と、**既に IDLE が動いている**場合 (再試行では
    直りません)
  - **404**: プロファイルが無いと確かめられた場合 (登録後に行が消えた場合を含む)
  - **403**: 委譲の認可が拒否された場合
  - **400**: 上記以外 — プロファイルが無効、IMAP 以外のコネクタ、パスワード未設定、
    **停止するセッションが無い**など、設定や要求そのものの問題
  - `GET .../idle/status` も、monitor が配線されていないノードでは **503** です (従来は
    空の一覧を返していました。「動いていない」と「このノードでは答えられない」は別です)
  - **IDLE で、取込が「取り込まなかった」と答えたメッセージを記録するようになりました。**
    従来はこの答えを捨てて「imported」とログしていました。対象フォルダが読めない等の拒否は
    例外ではなく結果として返るため、DLQ にも載らず、IMAP はそのメッセージを再配信しません。
    今は DLQ に**メタデータのみの行**を書き (取り直しのための記録で、再実行は中身が無いため
    失敗し行は残ります)、書けなければ `GET .../idle/status` の `undurableMisses` に数えます。
    取込内部の例外は取込側が既に DLQ に書くので、その場合は同じ行に二重に記録され
    `failureCount` が 1 多くなります。UID チェックポイントは動いていないので、復旧は
    メールボックスの再取得です
  - **IDLE で取込中に例外が出たメッセージ**（メールの取得失敗、取込処理から抜けた例外）も、同じく
    DLQ の記録行 (取得前なら「未読」の印付き) か `undurableMisses` に記録されます。従来はログだけで、
    IMAP は再配信しないため取りこぼしが見えませんでした
  - **委譲の再認可で、フォルダや ACL の読みが失敗した場合を「フォルダが無い」「cmis:all を
    失った」と記録しなくなりました。** 新しい拒否理由 `TARGET_FOLDER_LOOKUP_FAILED` /
    `CREATOR_CMIS_ALL_LOOKUP_FAILED` は「訊けなかった」の側で、IDLE はセッションを維持し、
    webhook は取りこぼしを記録します (従来は確定した拒否として IDLE を止め、監査に事実として
    載っていました)。scheduler も同じ理由で tick を飛ばし、自動無効化には数えません
- **webhook の非同期 fetch が設定やチェックポイントを読めなかった場合も、取りこぼしとして
  記録されるようになりました** (送信元には既に 200 を返しているため再送はありません。DLQ の
  記録行を書き、書けなければ `undeliveredWebhooks` に数えます)。この記録行は
  `webhookDeliveryRecord` という**行自身の印**で識別され、再実行では 409 で拒否されます。
  従来は `sourceObjectId` の接頭辞で識別していたため、同じ接頭辞を持つ本物の項目も
  拒否されていました
- **定義 API の `POST` で、`_all_docs` の走査が transport で失敗した場合は 503 になります**
  (従来は行が読めない場合と同じ 400)。行が読めない場合は従来どおり 400 です
- **配線の無いノードで「無い」と答えていた 3 か所を直しました**: webhook の購読
  (「No access token」400 → 503)、冪等性キー付きの取込 (「記録なし」として `replace` が
  削除しうる → 503)、チェックポイントの読み (「未取得」→ 拒否。取込は再試行になり、
  circuit breaker には数えません)
- **scheduler の poll で、1 つのプロファイルのコネクタ一覧が拒否されても tick 全体は
  止まりません** (そのプロファイルだけ「訊けなかった」として飛ばします)。フォルダ実行と
  手動起動の応答も、この場合 400「No compatible connector」ではなく **503** です
- **DLQ の書き込みが compare-and-swap になりました。** 保存・再実行の予約・確定書き込みは、
  読んだ時点の revision を条件に書きます。競合したとき、保存は読み直して再マージし (3 回
  負けたら「記録できなかった」)、予約は 429、確定書き込みは WARN を残して行をそのままにします。
  従来は書く時点の最新 revision を採用していたため、同じ項目への
  並行する保存の片方の欄が黙って失われ、再実行の予約は 2 つの同時要求が両方通っていました。
  再実行の **429「他の再実行が進行中」は、扉が行を読んでから書くまでの間に割り込んだ同時要求を
  確実に拒む答えになりました** (従来も内部の読み直しから書き込みまでの一瞬では起こりえましたが、
  扉の読みから書き込みまでの窓は守られていませんでした)
- **webhook 受信は、署名検証とレート制限の後に、コネクタを定義する行が 1 行だけであることを
  `_all_docs` の走査で確かめます。** 対 (索引に 1 行しか見えないものも) があれば 503 で、
  索引の並び順で選ばれた行の secret や有効状態では走りません。走査は署名検証の前には
  置きません (未認証のリクエスト 1 本で設定 DB を全走査させないため)。詳細は下の webhook
  受信の項
- **取込が返ったあとに fetch オーケストレータが張るリンク (チャットの添付とメッセージの
  親子など) も、委譲の再確認を通るようになりました。** 従来この経路はプロファイルを渡さず、
  取込中に委譲が取り消されていても辺を書いていました (検査に失敗したのではなく、検査そのものが
  行われていませんでした)。委譲プロファイルの場合、対象フォルダの現在の ACL に対して
  取込中のリンクと同じ確認を行い、通らなければリンクを作らずその旨を fetch のエラーに載せます。
  委譲でないプロファイルは従来どおり (再確認の対象外)。重複検査が答えられなかった場合に
  リンクを作って WARN に残す挙動は変えていません (fetch は失敗になりません)

  **この再確認が「いつの行」に対して行われるか**: 読み直すのは対象フォルダの ACL と
  コネクタの行で、**取込プロファイルの行はフェッチ開始時に読んだものを使います**
  (`isDelegated` と対象フォルダの指定は開始時の値です)。取込の後段で行う「欠けている
  属性の補完」は**現在の行**に対して問い直すので、2 つの経路で向きが揃っていません。
  プロファイル行を毎回読み直すとリンク 1 本につき設定 DB の走査が 1 回増えるため、
  3.4 では据え置いています。

  **一過性の読み失敗が拒否になりうる点**: 対象フォルダをパスで指定した委譲プロファイル
  では、この再確認がパス解決を通ります。解決の読みが一時的に失敗するとリンクは作られず、
  fetch のエラーになります (取込そのものは成立しています)。過剰に拒否していないことを
  確かめる自動テストは、現時点では**委譲でない経路しか押さえていません**
- **取込済みの項目を再度ポーリングしたときの「欠けている属性の補完」も、委譲の認可を
  問い直すようになりました。** この補完は取込本体が返った後に走るため、ポーリング中に委譲が
  取り消されても書き込まれていました。認可が通らない場合は補完せず、その旨を**警告**として
  応答に載せます (取込自体は失敗になりません。オブジェクトは有効だった認可のもとで
  取り込まれているためです)。コネクタ行が読めなかった場合も、委譲の確認ができないので
  補完しません。コストは補完 1 回につき、プロファイル行の読みが 1 回、委譲プロファイルなら
  さらにコネクタ行の読みと対象フォルダの `cmis:all` 評価が 1 回ずつです (対象フォルダを
  パスで指定しているプロファイルでは、その解決も。委譲でないプロファイルはプロファイル行の
  読みだけです)
- **取込 API の状態コードで、500 に落ちていた 2 つの拒否を分けました。** 重複判定のために
  対象フォルダを列挙しようとして**読みが答えなかった**場合は **503** (再試行で読み直せます)、
  列挙は返ったが**行が復号できず不完全**だった場合は **409** です (再試行は同じ壊れた行を
  読みます)。どちらも従来は 500 で、運用者には「サーバ側の不具合」と読めていました
- **multipart の取込で、取込そのものの失敗が 400「Invalid request」にならなくなりました。**
  multipart の入口は本文の解析と取込をひとつの catch で囲っていたため、取込が投げた失敗
  (ストアが答えない、取込フローの不具合など) まで「あなたのリクエストが不正」として
  400 で返していました。同じ失敗が JSON の入口では 500 です。解析だけを catch の中に残し、
  取込は外に出したので、**取込が投げたものについては両方の入口が同じ答えを返します**
  (型付きの拒否は従来どおり 503 / 409、それ以外は 500)。本文が壊れている場合、本文が JSON の
  `null` だった場合、100MB 超の場合は従来どおり 400 です
- **multipart でアップロードされた一時ファイルをこのノードが開けなかった場合は、400 ではなく
  503 になりました。** この入口が 400 で返す他の 3 つ (本文が壊れている・JSON の `null`・
  100MB 超) は送信側のバイト自身が示す事実ですが、一時ファイルはメソッドが動く時点で
  コンテナが既に保存し終えており、それを開く読みが失敗するのはサーバ側の理由
  (一時ディレクトリ、ディスク) です。従来は同じ「Invalid request」400 で、読みが何も
  確かめていないのに「あなたのリクエストが不正」と断定していました。アップロードし
  直せば保存もやり直されるため、500 ではなく 503 です。**開けた場合の取込は従来どおりです**
- **取込入口が「読めなかった」で返す 503 は、失敗の理由を応答に入れなくなりました。**
  対象は上の一時ファイルの読みと、コネクタ定義・取込プロファイルの行が読めなかった場合の
  503 です。どちらも**委譲の認可を確かめる前に**返るため、認証は済んでいるが取込を
  委譲されていない利用者にも届きます。一時ファイルの読み失敗はこのホスト上の絶対パスを、
  行の読み失敗は接続先ホストを、それぞれ含みうるものでした。**理由はサーバログに出ます**。
  応答はどの失敗かが分かる固定の文面で、状態コード (503) と「再試行」という指示は
  変わりません
- **セレクタ (Mango 索引) が失敗している間、webhook 受信は署名不一致・無効なコネクタ・
  URL 検証の 404 を、読めなかった読みと同じ 503・同じ本文で返します。** その窓では不在が
  503 になるため、401 を並べると「その id に読める行が在る」が未認証で分かっていました。
  正しい secret を持つ送信側はその窓でも通ります (一律拒否はしません)。プロトコル上の
  ハンドシェイク (Dropbox の GET challenge、Graph の `validationToken`) は署名より前に答える
  もので、この窓でも従来どおり存在を明かします
- **DLQ の新しい行は確定的な `_id` (`ingest_dlq:<dlqId>`) で書かれます。** 索引が再構築中で既存行が
  見えないときに同じ項目の 2 行目を作らず、書き込みが 409 で失敗して「記録できなかった」側に
  倒れます (このバージョンより前に書かれた行は生成 id のままで、その双子は依然起こりえます)
- **`DELETE /dlq/{dlqId}` で store が答えなかった場合は 503** です (従来は 500。行を探す読みと削除要求の両方、および設定 DB のクライアントが配線されていないノードを含みます)。
  行が返らなかった場合の 404 の本文から「retry」を外しました (行が無いのか索引がまだ追いついて
  いないのかはこの読みでは分からないため、事実だけを言います)
- **再実行が成功したあとの後片付けで store が答えなかった場合、500「Retry failed」ではなく
  200 `success-entry-kept` / `resolved-entry-kept`** で「取込は完了したが行は残った」と答えます
  (次の再実行が冪等に解消します)
...
- **チェックポイントの一括リセットが「全部消した」と答えなくなりました。**
  ...
  (プロファイルが無い場合も同じ答えです) は静的な scope しか消えませんが、応答は
  「All checkpoints reset」でした。今は消した本数を返し、定義行を読めなかったときは
  `warning` に「scope 付きのチェックポイント (slack/teams/mattermost/chatwork/m365mail/box)
  は名指せなかったので残っている」と書きます。`GET` 側も同じ `warning` を返します
  (503 にはしません — 静的な分は実際に消えているので、「何も起きなかった」と読める答えの
  方が実態から遠くなります)
- **スケジューラ付きプロファイルの作成・更新で、`defaultConnectorId` の行が読めなかった
  場合は 503 です。** 従来は「`defaultConnectorId 'x'` does not exist」という **400** で、
  読めなかっただけの行を「存在しない」と断定していました。本当に存在しない場合は従来どおり
  400 です

### コネクタ・取込プロファイル一覧の「続き」、webhook 受信、リンク作成が「読めなかった」を答えるようになりました

(一覧について「読めなかった」を答えるのは**一覧が完了できない**場合 — 続きのページ・
bookmark・応答・セレクタ呼び出しの失敗 — です。**逆直列化できない行 1 つ**は、管理 API の
一覧では従来どおり飛ばして残りを 200 で返します (この版から、その WARN ログに行の ID が
載ります — 以前は例外のメッセージだけでした)。1 行のために
一覧全体を 503 にすると、読める行の管理まで 1 行のために止まるためで、この版では変えて
いません — 飛ばした行の ID はその WARN に出ます。**呼出元のリポジトリを持つ (`repositoryId`
が一致する) 読めない行 1 行だけで定義されているプロファイルは、この版の管理 API では消せ
ません**: `?docId=` 付きの DELETE は分岐した対の片方を消す操作で単独の行を 409 で拒み、ID
なしの DELETE はその行を読めずに 503 になります。CouchDB 側でその行を直すか消してください
(`repositoryId` を持たない行は単独でも管理者が `?docId=` で消せます)。行単位で「読めない」を答えるのは、ID 指定の
読み書き (プロファイルの GET / PUT / DELETE / 所有権の確認、コネクタの GET)、webhook の
受信先解決とコネクタ解決、取込内の解決、そして「リポジトリごとに既定 1 つ」の規則が読む
4 欄 (下記) です。)

- **webhook 受信 (`/core/api/v1/ingest-webhook/{connectorId}`) の受信先解決が索引を読まなく
  なりました。** 従来は Mango セレクタで取込プロファイルを一覧していたため、**索引がその
  行を見せない状態では**「該当プロファイルなし」(`no_profile` / `no_matching_profile`、
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  場合は 500)。索引の再構築中に、在る行がそう見えるかどうかは**実機で測っていません** —
  確実に起きていたのは下記の 200 件上限です。今は `_all_docs` の走査で受信先を決め、
  走査が完了できないときは **503** を返します (送信側に再試行の余地を残す答えです。
  再試行するか・何回するかは送信側の仕様です)。この走査は webhook 1 イベントごとに
  設定 DB (`nemaki_conf`) 全体を 1 回読みます (上限は従来どおり 100 イベント/分/コネクタ)。
  - **コネクタ自身の解決も同じ扱い**です。コネクタの読み取りが答えられなかったとき
    (ID 直読みが失敗した、または行は在るのにコネクタとして読めない) は **503** です
    (従来は「署名不正」の **401**、または 500。401 は送信側に「あなたの署名が違う」と
    伝える答えで、送信側のログでは本物の署名失敗と区別がつかず、運用者が secret を疑う
    ことになります)。存在しないコネクタは従来どおり 401 で、**存在を確かめるための
    走査はしません** — この解決は署名検証より前に走るので、未認証のリクエスト 1 本で
    設定 DB を全走査させないためです。セレクタが失敗して確定的 ID の行も無いとき、
    セレクタが見せた行をこのノードが読めないときも 503 です (旧 ID の行を除外できない
    ため)。残るのは、旧 ID の行をセレクタが答えたのに返さなかった場合で、そのときは
    401 です — 再構築中の索引がそうするかは実機で測っていません。その行は直近の起動時
    移行が書き直せなかった行 (起動時に ERROR で報告されます) か、その後に旧ノードが
    書いた行 (次の起動まで報告されません) です。セレクタが同じ connectorId の行を 2 つ
    以上見せたときも 503 です (索引の並び順で秘密や有効状態が決まる側には倒しません)。
    **署名検証より前のこの読みが拒否する対は、セレクタが見せた対だけ**です。セレクタが
    失敗している間に確定的 ID の行が読めれば、署名はその行の secret で検証されます (旧 ID の
    相方はこの読みでは除外できません)。**署名検証とレート制限の後に、`_all_docs` の走査で
    その connectorId を定義する行がちょうど 1 行であることを確かめ、2 行以上なら (索引に
    1 行しか見えない対も、確定的 ID の行の隣の旧 ID の行も) 503 で受信を止めます。** 走査は
    Mango 索引に依らないので索引の再構築中も答え、走査が完了できないとき・索引が返した行を
    走査が示さないときも 503 です (本文は理由を言いません。理由はログにあります)。走査を
    署名検証の前に置かないのは、未認証のリクエスト 1 本で設定 DB を全走査させないためです
    (以前その位置に置いた版は欠陥として取り下げました)。したがって**相方の secret で署名
    されたイベントは 503 ではなく従来どおり 401** です。コストは、署名が通ったイベント
    1 件につき設定 DB の走査が 1 回増えること (受信先解決の走査と合わせて 2 回。上限は
    レート制限の 100 イベント/分/コネクタ)。この対は移行が回収しきれていない upgrade 済み
    環境にしか無く、管理 API の書き込みは従来どおり索引を使わない行数えで拒否されます。
    管理 API の `POST` / `DELETE .../subscribe` も同じ確認を通り、対があれば 503 です。
    Dropbox の URL 検証 (`GET`) も同じ解決を通り、答えられなかった読みは 404 ではなく
    503 です。
    **このコネクタ解決の答え (503 か否か) だけから未認証で分かること**を、答えが実際に
    分ける組で書きます。503 は「その id に受信側が拒否する行 (読めない行 — 確定的 ID でも
    旧 ID でも —、または 2 行以上) が在る」「ID 直読みが失敗した」「セレクタが失敗していて
    確定的 ID の行が無い」のいずれかで、どれかは分かりません。非 503 は「その id は不在」か
    「読める行が少なくとも 1 つ見つかり、2 つ目は見えなかった」(片方しか見えない pair や
    セレクタが見せない旧 ID の行は、この解決には見えません) のどちらかで、その後の答え
    (POST は無効な行でも署名不一致でも 401、GET は無効・Dropbox 以外・challenge 不備で 404)
    はこの解決だけでは不在と区別しません。**セレクタが失敗している間**、この解決の非 503 は
    「読める確定的 ID の行が在る」を意味します (不在はその間 503 になるため)。
    **その間、受信側はその差を答えに出しません**: 署名不一致・無効な行・GET の URL 検証の
    404 を、答えられなかった読みと**同じ状態コードと同じ本文**で返します (503 と
    `Connector could not be read; retry shortly`)。したがって、**下に挙げるハンドシェイクを
    除けば**、未認証の 1 リクエストからその id に読める行が在るかどうかは分かりません
    (ハンドシェイクは署名より前に答えるもので、この窓でも従来どおり明かします)。その間すべてを一律 503 にする
    (= webhook 全体を止める) 案は採らず、**正しい secret を持つ送信側はその間も従来どおり
    通ります**。代償は、その間だけ、署名が本当に違う送信側と無効なコネクタ宛の送信側が
    401 ではなく「retry shortly」を受け取ることです (再試行するか・何回するかは送信側の
    仕様です)。なお、受信側のプロトコル上のハンド
    シェイクは従来からこれと独立に存在を明かします: 有効な Dropbox コネクタは `GET` の
    challenge に 200 で答え、有効な teams / m365_mail コネクタは `validationToken` を署名
    検証の前に返します (Dropbox 側はコードの javadoc に以前から記載)
  - **受信先のプロファイル行が解釈できないときも 503** です。従来は解釈できない行を
    飛ばし、その行宛てのイベントを「該当プロファイルなし」(200) で受け取っていました。
    今は、飛ばした行の `defaultConnectorId` / `allowedConnectorIds` を読み、
    **受信中のコネクタを名指ししている行があれば**そのコネクタへの配送を拒否します —
    読める受信先が他に在っても、です (読める分だけに配送して 200 を返すと、壊れた行宛ての
    イベントが黙って消えるため)。別のコネクタを名指す壊れた行は、その配送を止めません。
    壊れた行の宛先は、`enabled` / コネクタの 2 欄 (`defaultConnectorId` / `allowedConnectorIds`)
    / `allowedArchetypes` を**本体と同じ読み方 (同じ mapper) で、問いごとに分けて**読んで
    決めます (コネクタの 2 欄は「誰を名指すか」という 1 つの問いに一緒に答えるのでまとめて、
    `allowedArchetypes` はそれとは別に読みます)。文字列を期待する欄では数値は文字列として、
    `enabled` の明示的な null は false として、list の null 要素はそのまま読み、mapper が読めない値 (知らない
    archetype 名、list でない `allowedArchetypes` など) だけを「読めない」とします (手書きの
    判定で mapper が読める値を「読めない」と扱い、確定できる除外を捨てていたのを直しました。
    **逆に、`enabled` を無効と読むのは mapper が読む形だけになりました** — 真偽値の `false`、
    文字列の `"false"` / `"False"` / `"FALSE"` (前後の空白は除かれます)、空文字や空白だけの
    文字列、`"null"` (前後の空白は同じく除かれます)、そして明示的な null です (空文字・空白
    だけ・`"null"`・明示的な null は「値なし」として false になります)。**数値で保存された
    `enabled` は読めません** — `0` も含めて「読めない行」として扱います。`"fAlSe"` のようにそれ以外の綴りの行は、従来
    「無効」として飛ばしていたのが「読めない行」として扱われます — その行は全体としても
    逆直列化できないので、無効かどうかを確かめられない側に倒しています。
    壊れた行の `allowedArchetypes` が読めて、そのコネクタの種類 (archetype) を明確に含まない
    場合も宛先ではないので止めません (読める行なら受信側の archetype 判定で外れる行です)。
    2 つの判定は欄ごとに独立です。コネクタの 2 欄のどちらかが読めない行は**すべてのコネクタ**
    を名指すものとして扱います (壊れた行 1 つで全 webhook が止まる側に倒しています — その行
    宛てのイベントが「該当なし」として消える側より、直すまで止まる側を取りました) が、
    `allowedArchetypes` が読めて除外できるコネクタは止めません。逆に `allowedArchetypes` が
    読めない行は全 archetype 宛てとして扱いますが、コネクタの 2 欄が読めて名指していない
    コネクタは止めません。
    `enabled` を false と読める壊れた行 (上に挙げた形 — 真偽値の `false`、3 綴りの文字列、
    空文字や空白のみ、`"null"`、明示的な null) は受信先になり得ないので数えません。行が直る (またはその値を読めるノードに切り替わる) まで続く状態なので、
    ERROR ログに行の ID が出ます。取込プロファイルのサービス自体が配線されていない場合も
    「該当なし」ではなく 503 です
  - `repositoryId` を持たない行は受信先になりません — 取込側がどのリポジトリの行としても
    解決しない行で、配送しても失敗するだけでした
- **コネクタ・取込プロファイルのセレクタ一覧に、200 件の無言の上限がありました。**
  `GET /core/api/v1/admin/connectors` と `GET /core/api/v1/admin/import-profiles`、および
  同じ一覧を内部で使う経路 (`/summary` / `/by-principal` / `/by-group`、archetype 別の一覧、
  書き込み前の既存行の確認。webhook の受信先も同じ上限を踏んでいましたが、こちらは上記の
  とおり走査に切り替えたのでこの項の対象外です) は、**201 件目以降を黙って
  落として**いました (索引再構築とは無関係に、毎回)。bookmark で続きを読むようになり、
  続きを読めないページ・同じ bookmark の再来・`docs` の無い応答・セレクタ呼び出しの例外
  (通信失敗のほか、この段で出る実行時例外) は拒否します。どの形でも原因は WARN ログに
  残ります (一覧側が自分で出す 3 つの拒否も含めて)。この拒否は、一覧を返す端点と一覧を直接使う
  経路 (コネクタ・取込プロファイル・フォルダ別コネクタの管理コントローラ) と、書き込み
  本体の既存行確認 (`POST` / `PUT` が保存前に同じ ID の行を探す段) では **503** です
  (従来、この 4 形のうち失敗として現れたのはセレクタ呼び出しの例外だけで、それは作成・
  更新を含め 500 でした。他の 3 形は拒否ではなく、黙った不完全な一覧でした)。
  1 件の読み取り (`GET` by id と、`PUT` の入口で定義を解決する段。`DELETE` はセレクタを
  読みません) はセレクタが答えなければ確定的 ID の直読みにフォールバックするので、そこでは
  一覧の不完全を理由にした 503 は出ません。索引不要にする変更ではありません — 索引が
  見せない行は、これまでどおり見えません
- **リンク作成の重複検査が答えられなかったとき、黙らなくなりました。** 既存の関係を
  読めなかった場合、リンクは従来どおり作ります (一過性の読み失敗で正当な初回リンクを
  止めない) が、取込の結果の `warnings` に「重複検査なしで作った — 重複エッジが出来た
  可能性がある」を返し、capture 記録にも同じ事実を載せます。従来は検査の失敗を
  「関係は無い」と同じ値にして、何も残していませんでした。関係の読み取りサービスが
  配線されていない場合も「無い」ではなく「確かめられなかった」です。**取込完了後に
  orchestrator が張るリンク (Slack / Teams / Mattermost) では、この事実は取得サマリには
  出ず、WARN ログに残ります** — そこでは非 null の答えがすべて取得の `errors` に入り、
  何も取り込まなかった取得が FAILED と記録されて circuit breaker が進むためで、作られた
  リンクはその失敗ではありません
- **「リポジトリごとに既定プロファイルは 1 つ」の検査が、規則が読む 4 欄 (`profileId` /
  `defaultConnectorId` / `enabled` / `defaultProfile`) だけを解釈するようになりました。**
  従来は行全体を読み、他の欄にこのノードが解釈できない値 (ローリングアップグレード中に
  新しいノードが書いた値、または破損) が 1 行あるだけで、そのリポジトリの**有効な**
  取込プロファイルの作成 (**400**) と更新 (**503**) が止まっていました (この検査は有効な
  プロファイルの書き込みにだけ走ります)。その行は今も規則に数えられます (既定が 2 つに
  なる経路は開いていません)。規則の 4 欄そのものが読めない行は従来どおり拒否します
- **取込の自動解決 (`profileId` を省いた取込) が、`enabled: false` の行を解釈しなくなり
  ました。** 従来は無効な行でも解釈できなければ 503 を返し、そのリポジトリの自動解決が
  全部止まっていました。無効な行はどの選択規則にも掛からないので、答えは変わりません
  (`profileId` の無い無効な行も同じで、従来はそれだけで書き込みを止めていました)。
  **有効**な行が解釈できない場合は従来どおり 503 です (その行が既定だったかもしれない
  ため)。「無効」と読むのは、本体と同じ読み方 (同じ mapper) で `enabled` が false になる値 —
  JSON の `false`、文字列の `"false"`、明示的な null など (完全な一覧は上記) — で、コネクタ側の
  一覧と同じ読み方です (`enabled` の**欄そのものが無い**行は無効とは読みません。行の
  `profileId` も同じ読み方で、数値は文字列として
  読みます — 一覧だけでなく、既存行の件数・ID 指定の読み・削除が行を探すときも同じ読み方
  なので、一覧が `"42"` と呼ぶ行は件数にも数えられ、その ID で読めて消せます。**コネクタの
  `connectorId` も同じです** — 従来はコネクタ側の一覧が `"42"` と表示する行を削除・件数が
  見つけられず、`DELETE .../admin/connectors/42` が成功を返したまま行が残っていました)
- **識別子が文字列でない行は、起動時の移行で文字列に直されます。** `profileId` /
  `connectorId` が JSON の数値などで保存されている行は、これまで「使える ID が無い」として
  移行対象外でしたが、本体はその値を `"42"` として読みます。移行時に確定的 ID
  (`import_profile_definition:42` / `connector_definition:42`) を与え、**その欄自体も読んだ
  文字列に直して**書きます。直さないと Mango セレクタ (型に厳密) がその行を一生見つけられず、
  件数だけが 1 を返すため、**そのプロファイル / コネクタの更新が「索引が追いつくまで待って
  ください」(503) を返し続ける**状態になります (索引の再構築では解消しません)。移行が走るまでの
  間はこの状態になり得ます。**既に確定的 ID に居て欄だけが文字列でない行**も、同じ ID のまま
  読んだ revision 条件付きで書き直します。ただし**添付を持つ行は書き直しません** — この移行は
  添付を複写しないので、書き直すと添付が消えるからです。その行は起動ログに ERROR で名指され、
  更新は 503 のままです。`repositoryId` が文字列でない行は従来どおり移行しません
  (どのリポジトリにも属さない壊れた行です) が、案内どおり `?docId=` で削除できるように
  なりました (従来は案内された API が 404 を返していました)
...
## 記録を E-ARK SIP として書き出せるようになりました

1 件の記録を **E-ARK CSIP 2.2.0** のパッケージ (zip) にして返す admin 専用の口が付きました。
別の組織や保存システムへ渡すための形式です。

```
POST /core/api/v1/admin/eark/export?repositoryId=...&objectId=...
POST /core/api/v1/admin/eark/{repositoryId}/objects/{objectId}/bag?submissionId=...
```

**運用に影響する点。**

- **admin 専用です。** パッケージには記録の本文とメタデータが入り、渡したら取り消せません
- **INTERNAL_ONLY と印を付けた「属性」は既定で入りません。** `includeInternalOnly=true` を
  明示したときだけ入ります。省略時は入りません — 既定が漏らす側だと、誰も選んでいない
  開示になります
- **ただしこれは属性の話で、本文の話ではありません。** この指定は**メタデータの属性を
  選ぶだけ**で、**文書の本文は常に入ります**。本文そのものに個人データが含まれるなら、
  `includeInternalOnly` を省略しても出ていきます。**渡してよい記録かどうかは、
  このフラグではなく渡す前に判断してください**
- **伏せた属性の件数が `X-Nemaki-Withheld-Property-Count` ヘッダに出ます。**
  zip をそのままディスクへ流す呼び出し元は本文中の注記を読まないので、ヘッダに出しています
- **同梱のバリデータを必ず通します。** 動いて却下したパッケージは**返しません**。
  バリデータ自体が動かなかった場合は拒否せず「検査できなかった」と明記して返します —
...
- **E-ARK 準拠とは名乗りません。** 通ったのは `commons-ip2` に同梱された検証器であって、
  DILCIS Board の公式サービスではありません。言えるのは「この版のこの検証器を通った」までです
- **どの保存システムでも受け取れる、とは言いません。** 実際に測ったのは RODA 6.3.0 と
  Archivematica 1.18.0 の 2 つだけで、しかも**受け取り方に注意が要ります** (次項)
...
- **RODA には SIP を直接渡してください。bag ではありません。**
  ...
  「この製品は E-ARK 非対応だ」に見えるので注意してください
- **RODA の bag 取込には渡さないでください。** 本製品の bag は payload manifest を 2 本
  持ちます (RFC 8493 が許す形)。RODA の bag 経路はそれを扱えず、**取込全体が rollback します**
- **RODA は本製品の PREMIS を AIP に残しません。** 本文・`dc.xml`・同梱した JSON 2 本・
  タイムスタンプ (`ers.der`) は残りますが、PREMIS 文書は RODA 自身の PREMIS に置き換わります
  (**この `ers.der` はスタブの DER で測っています。本物の RFC 3161 ベースの証跡では未測定**です。
  取り込まれるかどうかを決めているのは METS のどの枠に宣言したかで、**中身の種類ではない**
  ことまでは測れているので、本物でも同じになる公算が高い、というところまでです)
- **置いた場所と、残る場所は違います。** 本製品が `metadata/other/` に入れたものを、
  RODA は **`metadata/descriptive/` へ移します**。AIP を受け取った側が `other/` を
  探しても見つからないので、**引き渡しの取り決めに書いておいてください**
- **Archivematica は bag でも zip でも AIP になります。** BagIt は必須ではありません。
  それでも bag を推すのは、**`zipped bag` で投げたときだけ `Verify bag` が走り、本製品の
  SHA-256 が照合される**からです (比べたのは `zipfile` と展開 `standard` です。
  `unzipped bag` は測っていません)
- **Archivematica は本製品が同梱した manifest を AIP の中に残します** (sha256 / sha512 とも)。
  置き場所は `data/objects/metadata/transfers/{転送名}-{転送 UUID}/` で、**この UUID は
  AIP の UUID ではありません**。なお AIP の直下にも同じ名前の manifest がありますが、
  そちらは **Archivematica が自分の AIP について作ったもの**です
- **取り込まれたことは、保持されることでも、E-ARK として理解されたことでもありません。**
  RODA の AIP は受入承認前の状態で止まり、Archivematica の AIP は Archivematica の AIP です

## 管理責任の移管を記録できるようになりました

**「報告されたこと」と「こちらが確かめたこと」を別の状態として持つ** 9 段階の記録です
(これに加えて、custody が渡る前ならどこからでも到達する `FAILED` があります)。
受領証は証拠の連鎖に載り、載せられなければ**移管は成立しません**。

この版に送信経路は無いので、**前半は「誰かが記録したこと」までです**。
署名を検証できたときに言えるのは **「渡された鍵の持ち主がこの受領証を作った」**までで、
その鍵が本当に相手組織のものかどうかは、引き渡しの取り決め側の話です。

```
POST /core/api/v1/admin/custody/{repositoryId}/transfers
POST /core/api/v1/admin/custody/{repositoryId}/transfers/{transferId}/receipt
POST /core/api/v1/admin/custody/{repositoryId}/transfers/{transferId}/pass-custody
```

**運用に影響する点。**

- **受領証は、こちらが送ったパッケージを名指していないと受け付けません。**
  「作った AIP の checksum はこれです」だけの受領証は、見たことのない成果物の hash なので
  **どんな値でも条件を満たしてしまいます**
- **署名は運ぶだけで、検証は鍵があるときだけです。** 鍵が無いのは「検査していない」で
  あって不正ではなく、`signatureVerified` は既定 false のままです
- **移管の記録が書けなければ、移管しません。** 取込とは逆向きの判断です — 取込は既に
  起きているので拒否できませんが、移管は**まだ起きていない**ので、拒否の代償は再試行だけです
- **移管の行は証拠台帳と同じ `nemaki_evidence_ledger` に入ります。** 1 つの記録の来歴は
  1 つの retention policy の下に置きたいので、DB は分けていません

### 受け取り側の言葉は、そのままでは通りません

RODA は `SUCCESS`、Archivematica は `COMPLETE` と言います。**本製品が「成功」と認めるのは
`PASSED / PASS / VALID / SUCCESS / ACCEPTED / OK` だけ**なので、Archivematica の `COMPLETE` は
そのままでは**正当な受領を拒否します**。語彙は増やしません — 3 つ目の受け手が同じ語を別の
意味で使ったときに、今度は**通してしまう**側へ倒れるからです。代わりに接続層が写像し、
**先方が実際に言った語も併せて残します**。

### 主張しないこと

- **送信はまだできません。** パッケージを受け手へ渡す HTTP はこの版にはありません。
  在るのは記録の型と規則、bag を作る口、そして受領証を組み立てる部品までです
- **移管したから安全になった、とは言いません。** 記録が言うのは「引き渡しを記録し、
  その受領証を検査した」までで、**先方が今も持っている**ことではありません
- **署名の無い受領証が establish するのは、「この製品が、我々のパッケージを名指す陳述を
  受け取った」までです。**
- **署名が検証できても、「先方が本当に取り込んだ」ことにはなりません。** 検証が言うのは
  **「渡された鍵の持ち主がこの受領証を作った」**までです。その鍵が本当に相手組織のもので
  あることも、受領証に書かれた内容が真実であることも、**署名は何も言いません**。
  鍵をどこから得てなぜ信じるかは、引き渡しの取り決め側の話です

## 「読めなかった」を「無かった」と答えるのをやめました (**挙動変更**)

一覧・件数・検索の結果が**空**で返ってきたとき、それが「本当に無い」なのか
「読めなかった」なのかを、呼び出し元が区別できない箇所が多数ありました。
CouchDB の view が答えなかった、行は返ったが文書が付いてこなかった、例外が出た —
いずれも**空のリストや 0** になり、その上で
「このフォルダは空です」「未解決は 0 件です」「この記録は連鎖に入っていません」と
断定していました。

**これらは拒否するようになりました。** 影響が出るのは次の形です。

- **フォルダの一覧・件数が読めないとき、空ではなくエラーになります。**
  CMIS のナビゲーション、`/fixity/scan/folder`、カタログの巡回が対象です。
  **本当に空のフォルダは今までどおり空**を返します
- **`/fixity/scan/folder` に `status: "partial"` が増えました。** フォルダは在るが
  子の一部が読めなかった場合です。`unreadableChildren` には**読めなかった子がある限り**
  件数が出ます — `status` が `"partial"` のときも、検証そのものが失敗した (`"error"`) ときも
  です (読み切れたときはキー自体が出ません)。応答の `scope` / `scopeLimits` も変わり、
  **証拠連鎖に書かれる scope も変わります。4 通りあります**:

  | 検証の結果 | 読めなかった子 | scope |
  |---|---|---|
  | 完走 | 無し | `folder-children:{id}` |
  | 完走 | 有り | `folder-children-partial:{id}:unread=N` |
  | **失敗** | 無し | `folder-children-incomplete:{id}` |
  | **失敗** | 有り | `folder-children-incomplete:{id}:unreadAtLeast=N` |

  `partial` は「意図して範囲を狭めた」ときの語なので、途中で落ちた検証には使いません。
  `unreadAtLeast` は「読めなかった N 件**＋どこまで進んだか不明な分**」の下限です。
  scope で検索している運用があれば **4 つとも**追加してください —
  `incomplete` を落とすと**いちばん見つけたい「落ちた検証」だけが漏れます**
- **`/fixity/scan/folder` の HTTP コードが変わりました。** 本文を開かず
  コードだけ見る監視は反応します:
  検証が失敗 → **500** (以前は 200 + `status:"success"`)、
  解決しない id → **404** (以前は 200 + `verdict:COMPLETE` / `scanned:0`)、
  フォルダでない id → **400**
- **`scopeLimits` が言えることの範囲も明記しました。** 数えているのは
  「リポジトリが返した一覧のうち読めなかった行」で、**他のノードが後から変えたために
  キャッシュ側の一覧が古い**場合は検知しません。一覧についての陳述であって、
  データベースが今持っているフォルダについての陳述ではありません
- **E-ARK パッケージの `nemaki-evidence.json` に `undecodableEntries` が出ることがあります。**
  連鎖の行を一部読めなかったという意味で、**「連鎖に入っていない」ではありません**
- **lineage journal の一覧・投影が、到達できないときは 1 周を飛ばします。**
  定期実行そのものは止まりません
- **取込イベントの重複判定が実行できなかった場合、追記せずに dead-letter へ回します。**
  追記専用の journal に二重に入るより、ファイルに残して後から再生する方を採りました
- **ゴミ箱 (アーカイブ) の一覧と件数が読めないとき、空ではなくエラーになります。**
  今までは一覧が短く返り、件数は 0 になり、**ゴミ箱が空として描画**されていました。
  復元・完全削除の判断がこの一覧に乗るため、短い答えのまま進めません
- **CMIS の変更フィード (`getContentChanges`) は、読めない変更行があると
  エラーになります。** 今までは抜けた行を**飛ばして `changeLogToken` を進めて**
  いました — 進めた先の変更は、そのクライアントには二度と配送されません。
  再試行すれば同じ位置から読み直せます
- **変更フィードの 1 ページは最大 10,000 件になりました。** 巨大な `maxItems`
  (例: `Integer.MAX_VALUE`) は今まで**静かに空のフィード**になっていました —
  CouchDB が上限超の limit を拒否し、そのエラーが空リストに化けていたためです。
  今は最大 10,000 件と継続用トークンが返り、続きからページングできます
  (返るトークンは「返した最後のイベント」の位置なので、クランプで取り残される
  ことはありません。`hasMoreItems` は「リポジトリの最新に達したか」で決まるため、
  続きがあるときだけ true です)。**`maxItems=0`・負値・int 範囲外の値も同じ
  1 ページ扱い**になりました — 今までは「無制限」に化けて、変更ログ全件を
  一度に読み込もうとしていました (int 範囲を超えた値は符号が壊れて 0 や負に
  切り詰められる経路もありました)
- **ユーザー・グループの一覧・件数・所属解決が読めないとき、空ではなく
  エラーになります。** 所属グループの解決 (直接・入れ子とも) が短い答えを返すと、
  **本人には何も表示されないまま権限だけが減ります** — 方向としては安全ですが、
  管理者が気づけません。ユーザー・グループ削除時の逆引き (どのグループが
  参照しているか) も同じで、短い答えは**宙ぶらりんの参照を残す削除**になります
- **チェックアウト一覧・バージョン一覧・関係の一覧・適用ポリシーの一覧も同じです。**
  特に適用ポリシーは、読めない行を黙って外すと**その文書への制御が静かに
  外れて見える**ため、拒否に倒しています
- **型定義の一覧が読めないとき、「基本 2 型だけ」を合成しなくなりました。**
  今までは一時的な障害でも `cmis:folder` と `cmis:document` の 2 型だけを返し、
  外部カタログ同期がそれを「カスタム型は全部消えた」と読んで**外部側の型エンティティを
  削除**する経路がありました (作り直しでは分類・用語は戻りません)。エラーになります。
  **view が「答えて 0 行」の場合だけは今までどおり基本 2 型で立ち上がります**
  (通常は初期構築前だけの状態ですが、型文書が実際に消えている場合や、map 関数が
  失敗して 200 + 0 行を返す場合も同じ扉です — 後者は「主張しないこと」に挙げている
  既知の盲点で、この節の防御では塞げません)
- **オブジェクト取得の失敗が「存在しない」と同じ値ではなくなりました
  (キャッシュ層まで含めて)。**
  CouchDB の一時障害が「このオブジェクトは無い」として返り、**不在を根拠に動く処理**
  (削除トゥームストーンの解決が外部カタログのエンティティを消す、アーカイブの突き合わせ、
  ユーザー/グループ削除時の参照剥がし) がそれを信じて動いていました。
  失敗はエラーになり、null は「本当に無い」だけを意味します。
  この規則は**キャッシュ層・型付き読み取り・バージョン系列・一括読み取り**にも
  適用されました — 直下の層だけ直しても、その上のキャッシュが失敗を null に
  戻していたためです。**設定の不備 (キャッシュや DAO の未配線) も
  「オブジェクトが無い」ではなくエラー**になります。**一括読み取り**は、
  CouchDB が「無い」と答えた行 (`not_found` / 削除済み) だけを飛ばし、
  それ以外の失敗行やバッチ全体の失敗ではエラーになります — 今までは
...
- **ユーザー/グループ削除は、参照元グループを読み直せないとき中止されます。**
  ...
  削除済み principal の ID が残りました — **同じ ID を後で作り直すと、その
  メンバーシップが承認なしで復活**します
- **ゴミ箱の数え直し・ページ読みの失敗が、下の層で 0 や空ページに握り潰されなくなりました。**
  前版で入れた「失敗した件数は 0 ではない」の防御は、その一段下の CouchDB
  ラッパーが先に 0 を返すため**実際の障害では一度も発動しない**位置にありました
- **RSS フィードの `limit` と `maxDepth` に下限・上限が付きました。**
  `?limit=-1` は「空のフィード」の顔をした無制限クエリでした。maxDepth は上限 16 です
- **RSS トークンの一覧・取得が失敗を握り潰さなくなりました。** 一覧が途中で
  失敗すると**読めた分だけ**が返り、管理者が「全部失効させた」と誤認できました。
  取得失敗は「Token not found」ではなくエラーになります
- **フォルダ一覧・子孫取得の `maxItems` / `skipCount` / `depth` に上下限が付きました。**
  int の範囲を超える値は今まで**符号ごと切り詰められ**、2³¹ は負の値に、2³² は 0 に
  なっていました（「全部くれ」が「何もくれるな」や「負のページ」に化ける形です）。
  1 ページは最大 10,000 件で、続きは `hasMoreItems` とページングで取得します。
  `depth = -1`（無制限）の意味は変わりません
- **型定義の一部が読めないとき、その型を黙って外した型体系を提供しなくなりました。**
  型レジストリを作り直す途中で失敗したときも、基本型だけの状態で「その型は無い」と
  答えずエラーになります (起動時・動的初期化・強制再読込の 3 経路)。
  今までは 1 型ずつ静かに欠け、外部カタログ同期がそれを「その型は消えた」と
  読む経路が残っていました。型レジストリの**組み立て側**も同じで、組み立てに失敗した
  型とその配下を抜いたまま起動を完了することはなくなりました
- **型一覧・検索の `maxItems` / `skipCount` / `depth` にも上下限が付きました。**
  なお `maxItems` を**省略した**ときの既定はサービスごとに異なります
  (フォルダ一覧 100 件 / 型一覧は全件相当 / 検索は認可済み全件)。統一したのは
  0 や負値を渡したときの扱いで、省略時の既定までは揃えていません。
  フォルダ一覧と同じ切り詰め (2³² が 0 になる) が残っており、**型一覧が空**で返る、
  検索が**空ページ**で返る、という形になっていました
- **ユーザー・グループ・ポリシーの取得失敗も「存在しない」ではなくエラーになります。**
  今までは一時的な障害が 404 として返り、ディレクトリ同期はそれを
  「そのアカウントはもう無い」と読んでいました
- **保持期限のスキャンが失敗したとき、「対象 0 件で完走」と記録しなくなりました。**
  実行ログには失敗として残ります (誤って削除する方向ではありませんが、
  監査記録としては逆のことを言っていました)
- **ディレクトリ同期の孤児削除が、通常の削除と同じ経路を通るようになりました。**
  特にグループ側は**参照の剥がしを一切行っておらず**、削除したグループの ID が
  親グループの入れ子リストに残っていました (同じ ID を作り直すと復活します)
- **RSS トークンの検証がプロセス内キャッシュを見なくなりました。**
  複数レプリカ構成で、あるレプリカで失効させたトークンが**他のレプリカでは
  そのまま使えていました**。キャッシュを廃止し、毎回ストアを読みます
- **起動時だけの緩和が、スレッド名の推測ではなく明示的な区間になりました。**
  今までは実行スレッドの名前に "main" 等が含まれるかで判定しており、
  コンテナが付けた名前次第で**通常のリクエストが起動時扱いになる**可能性がありました
- **RSS / Atom フィードは、読めない行があると 500 を返します。**
  フィードには変更ログのようなトークンが無く、購読者は最新 N 件の窓を読むだけです —
  窓から黙って抜けたイベントは**再配送される機会そのものがありません**。
  一般的なフィードリーダーは 500 を「後で再試行」として扱い、手元の既読分を保持します
  (全リーダーの挙動を保証するものではありません)。
  フォルダ購読の対象サブツリー集めも同じで、読めない子行があるときは
  そのサブツリーを黙って外さず、フィード生成を拒否します

### 主張しないこと

- **これで「読めない状態」が無くなるわけではありません。** 変わったのは、
  読めなかったことが**答えに出る**ことだけです
- **いちばん危険な形は、これでは塞げません。** CouchDB の view は map 関数が失敗しても
  **HTTP 200 と 0 行**を返すので、例外が出ません。**空のリストは空のフォルダと
  区別できません。** 今回塞いだのは「view が答えを返さなかった」場合と
  「行は返ったが読めなかった」場合です。0 行の場合を捕まえるのは、
  再索引が持っている**「索引が既に持っている件数」との突き合わせ**の方で、
...
CMIS の全再索引には以前から歯止めがありました — **索引が既に持っている件数**と
これから入れる件数を比べ、桁違いに少なければ**消さずに拒否**します。
CouchDB の view は map 関数が失敗しても 200 と 0 行を返すので、
「repository が空に見えるだけ」と「本当に空」を例外では区別できないためです。
実際に 164 オブジェクトの repository が 1 件になり、`errorCount: 0` /
`status: completed` と報告された事故が起きています。

**RAG 側にはこの歯止めがありませんでした。** 同じものを入れました。併せて:

- **途中で歩けなかったフォルダが `errorCount` に出るようになりました。**
  今までは上限付きの `errors` 一覧に足すだけで、集計値は 0 のままでした
- **落としたフォルダがあるときは `completed` ではなく `completed_with_errors`** です。
  索引を消した後に一部を飛ばした実行は、完了ではありません

`POST /api/v1/cmis/repositories/{repo}/search-engine/rag/reindex` は
アップグレード時の必須手順として案内しているものです。**拒否された場合は、
索引は消えていません。** 本当に縮んだのであれば明示的に消してから実行してください。

**読めない子行があるフォルダは、ツリー削除で親を消しません。**
これは CMIS の全バインディング・REST が通る**本番の削除経路**
(`deleteTree` の DFS と、通常の `deleteObject` のフォルダ枝の両方) に入っています。
併せて、CMIS の `continueOnFailure=false` が**受け取られたまま無視されていた**のを直しました
— 今は指定どおり、失敗を観測した時点で以降の削除を試みず、
**試みなかった直下の兄弟も `failedToDelete` に列挙**します (仕様上この一覧は
「削除されなかったオブジェクト」なので、省略すると削除済みに読めてしまいます)。
未試行フォルダの**中身**までは列挙しません — 歩かなかった木の中身は数えられないためで、
そのフォルダ自体が一覧に載ることが「その下も消えていない」の印です。
`false` 指定時の同階層ファイル削除は並列ではなく逐次になります。
保持されたフォルダを**検索索引から消してしまう**穴も塞ぎました —
安全のために残したフォルダが、検索には存在しない状態になっていました。
削除待ちの並列処理が**割り込まれた場合**も、状態不明の子を「削除済み」と
みなさず親を保持します。
同じ「短い一覧」への防御を、**名前で探して無ければ作る**経路すべて
(ユーザー/グループ用フォルダの作成 2 系統、一括チェックイン、クラウドドライブ保存、
zip インポートの重複判定) に入れました — 読めない行があるときは重複を作らず拒否します。

**Purview 同期の「読めない行があった回」の基線が、少しだけ賢くなりました。**
読めない行があった回は今までどおり**何も削除しません**が、その回に**発行できたもの**
(新しく現れた辺・文書) は基線に加わるようになりました。以前は基線が完全に据え置かれた
ため、「不完全な回に作られ、完全な回が来る前に消えたもの」が外部カタログに残り続ける
穴がありました。発行は 1 文書ずつ行われ、**載った文書だけ**が基線に入ります —
発行に失敗した文書は基線に入らず (既存文書は前回の値のまま)、次回に retry されます。
これは「読めない行があった回」だけでなく**通常の回でも同じ**で、以前は通常の回が
発行失敗した文書ごと基線を進めてしまい、その文書は内容が変わるまで再発行されません
でした。

**Purview への増分同期**の子孫展開も同じ防御を持ちますが、形が違います。
読めない行のあるフォルダは**その回の失敗として計上**され
(ジョブは `COMPLETED_WITH_ERRORS`)、その**子孫の展開は行われず**
(フォルダ自身の変更は発行されます — 自分のパスは親から正しく計算できます)、
他の変更の同期は**止まらずに続きます**。該当フォルダは dead letter に残りますが、
**dead letter の再試行はこのサブツリーのパス再計算を再実行しません** —
完全に直すには、壊れた行を修復したうえで **full sync** が必要です。
失敗メッセージにも同じことが書いてあります。
`deleteTree` は見えた子を消してから親を消しますが、リポジトリが decode できない行は
**その一覧に最初から居らず**、例外も出ません。今までは親だけが消え、読めない子は
「id では実在するのに、もう存在しないフォルダにぶら下がる」孤児になりました。
削除には後から拾い直す仕組みがないため、読めない子が居るフォルダ自体は削除せず、
`failedToDelete` に載せて理由をログに出します。併せて、**入れ子フォルダの削除失敗が
親に伝わっていなかった**のを直しました (子孫が残っているのに祖先が消える、同じ形の穴です)。

**再索引も「読めなかった子」を失敗として数えるようになりました。**
decode できない行は例外なしで一覧から抜けるため、今までの再索引はそこを**黙って
通り過ぎ**、`completed` で終わっていました — その文書は誰にも知られないまま
検索から消えたままになります。CMIS・RAG の両方で、抜けた行数が `errorCount` と
`errors` に載り、結果は `completed_with_errors` になります。

**CMIS の全再索引・フォルダ再索引も同じ言葉を使うようになりました。**
`errorCount` が 1 以上なら `completed_with_errors` です。これは
「文書の索引付けに失敗した」だけでなく、**歩けなかったフォルダがあった**
(サブツリーが丸ごと索引に入っていないかもしれない) も含みます。
今までは `errorCount` がいくつであっても `completed` で、画面上は
成功と同じ緑のタグでした — 件数はすぐ隣に出ているのに、色が矛盾していました。

**エラー本文は 100 件までです。** 超えた分は件数にだけ入ります。
今までその差の説明がどこにも無く、`errorCount: 5000` の隣に 100 件の一覧が
並ぶだけでした。一覧の末尾に「以降は件数のみ」と出るようにしました。

ただし**再索引後の health check の不一致はこの言葉に含めません**。
CouchDB 側の数え方が再索引と同じ木の走査なので、走査が短くなれば
両側とも短くなり、不一致が独立した証拠になりません。加えて
フォルダ単位の再索引でも health check は**リポジトリ全体**を見るため、
既存のずれがあるだけで毎回 `completed_with_errors` になってしまいます。
結果は今までどおり `errors` 一覧と、画面の health カードに出ます。

## 引き渡し先へ問い合わせたまま止まらなくなる状態を塞ぎました

移管先 (RODA / Archivematica) から digest を回収するとき、
**受け手が応答を始めてから黙り込むと、その読み取りは止まりませんでした。**
リクエスト側のタイムアウトはヘッダが返った時点で満たされてしまい、
本文の読み取りには効いていなかったためです。サイズ上限はありましたが、
毎秒 1 byte なら上限に達するまで何十年もかかります。

本文にも別途 10 分の上限を設けました。超えた場合は
「受け手が答え始めてから止まった」と明示して中断します
(「ネットワークが切れた」とは区別されます)。

## 引き渡しの記録が、確かめていないことを言わなくなりました

- **打ち間違えた object id** で単体検証を呼ぶと、HTTP 200 と `status: "success"` が
  返っていました。id が何も指していないだけで、バイトについては何も調べていません。404 です
- **payload 名を渡さずに** manifest を照会すると、実際に受け手へ問い合わせたうえで
  「受け手の manifest はこの transfer が送った package を記述していない」と返っていました。
  問い合わせ側が引数を渡していないだけです
- **manifest の行が SHA-256 かどうか**を見ていませんでした。`manifest-md5.txt` を
  指してしまうと 32 桁の MD5 がそのまま SHA-256 として扱われ、比較に負け、
  「受け手が別の package を持っている」という拒否になっていました
- **失敗した検証の記録**が、追記専用の鎖に「一部のみ (未読 3 件)」と書かれていました。
  「一部のみ」は意図的に範囲を狭めたときの言葉で、途中で落ちた検証には使えません
- 引き渡し履歴の「N 件読めませんでした」が、**問い合わせ自体ができなかった場合も 1 件**と
  数えていました。この 2 つは別の事実なので分けました — 問い合わせができなかったときは
  「**照会できなかったので、何件あるかは不明**です。在るという判定でも、
  無いという判定でもありません」と返します。
  (一度「少なくとも N 件」に直しましたが、**「1 件以上」もまた存在の主張**です。
  在るとは限りません)

## E-ARK SIP の証拠ファイルが、無いものを証明したと言わなくなりました

`nemaki-evidence.json` は組織の外に出て、後から訂正できません。

- **監査経路が無い package でも**「監査経路が…を証明する」という但し書きが付いていました。
  経路を作れた場合にだけ付くようにしました
- **鎖の行が読めなかった**ことと**取り込み記録が読めなかった**ことが**排他**扱いで、
  両方起きると前者が package から消えていました。両方載ります
- **何も確立できなかった重い失敗**にだけ通知がありませんでした
  (zip を直接保存する利用者には JSON が見えないため、ヘッダで通知しています)
- 取り込み記録のうち**使えなかった行**が黙って捨てられ、
  「取り込み記録は見つからなかった」と書かれていました

## 長期検証で「この版が知らないアルゴリズム」を記録の欠陥として報告しなくなりました

証拠記録 (RFC 4998) の検証で、この版が知らないダイジェストアルゴリズムや
JVM が持っていない実装に当たると、**「タイムスタンプが別のものを覆っている」**
という記録側の欠陥として報告していました。読み手側の限界です。
「確認していない」に分類されます。

## 型の削除が、規模の大きいリポジトリでも現実的な時間で終わるようになりました

「この型のオブジェクトがまだ在るか」を確かめる問い合わせ**2 本**に、どちらも
**索引がありませんでした** (主型を見る問い合わせと、副次型として使われていないかを
見る問い合わせ。前者が空振りしたときは**必ず**後者へ進みます)。
CouchDB は全文書を走査するので、**規模が大きくなるほど遅くなり、ある大きさから
タイムアウトします**。「どんな規模でも」ではありません — 小さなリポジトリは両方とも
問題なく答えます。80 万文書の環境での実測では、主型の問い合わせは 46 秒
(上限内だが十分遅い)、副次型の問い合わせはタイムアウトしました。
確認できなければ削除を拒否する作りなので (実体の在る型を消さないための設計です)、
**その規模では型の削除ができなくなり**、しかもメッセージは一時的な障害のように読めます。

本製品自身の TCK が、テスト用リポジトリが 80 万文書を超えたところで踏みました。

**アップグレード時に、各リポジトリの DB へ JSON 索引が 2 つ追加されます**
(`type-dependency-indexes/idx_objectType` と `idx_secondaryIds`)。
**索引は書き込みのたびに更新される**ので、投入の多い配備ではその分の負荷が増えます。
初回の構築は文書数に比例し、**構築中の問い合わせは遅いまま**です
(80 万文書の環境で数分。構築が終われば `objectType` は即答、`secondaryIds` は 1〜2 秒でした)。
**この 1〜2 秒はその規模での実測**です。全文書が `secondaryIds` を (空でも) 持つため
索引は疎にならず、`$elemMatch` は索引の走査になるので、**所要時間は文書数に比例します** —
一桁大きい環境では同じ形に戻りうると考えてください。索引の作成は起動時のパッチで行われ、
失敗しても起動は止まりません — 次回起動で再試行します。**索引が無いリポジトリも
通常の読み書きは動きます**。できないのは型の削除だけです。

### 主張しないこと

- **これで型削除が必ず成功するとは言いません。** 実体の在る型は今までどおり拒否されます。
- **「確かめられずに拒否される」が無くなったとも言いません。** 索引は問い合わせを
  速くしますが、上の段落のとおり**所要時間は文書数に比例したまま**なので、
  一桁大きい環境では同じ形に戻りえます。変わったのは、80 万文書規模で実測した
  タイムアウトが解消したことです

## E-ARK の書き出しが一時ディレクトリを残さなくなりました

`/eark/.../export` と `/bag` は呼ぶたびにシステムの一時ディレクトリに作業用の
ディレクトリを作り、**一度も削除していませんでした**。拒否 (409) やエラーでも残るため、
**繰り返し叩く運用ほど溜まります**。ダウンロードを流し終えた時点、および
パッケージを返さなかった時点で削除するようにしました。

## 変換 (rendition) を証跡に残すようになりました

- **rendition を永続化する口が 1 つになりました。**
  CMIS 経路も REST の rendition 3 本もコピー時の rendition 複製も、同じ内部経路を通ります。
  「変換済みのバイト列を渡して永続化する」メソッドは**消しました** — 自分が作っていない
  コピーを渡す方法が無いので、**記録し忘れる書き方ができません**
- **ただし「必ず記録する」とは言えません。** 記録器 (bean) が配線されていないノードでは
  rendition は作られて**記録は残りません**。以前は無言でしたが、
  そのときログに WARN を出すようにしました。変換自体は止めません
  (証跡の bean が無いことを理由に変換を拒否するのは釣り合いません)
- **PDF/A の適合判定 (veraPDF) を配線しました。** `rendition.pdfa.validate.flavour` を
  **設定したときだけ**動き、判定は複製記録の digest にコミットされ、報告にも出所付きで出ます
- **flavour は 2b か 3b にしてください。** コンテナ内の LibreOffice 24.2.7.2 で実測した
  範囲では 2b / 3b は適合し、**1b は落ちます** (日付表現の不一致 1 点)。1b を設定すると
  全コピーが正しく DOES_NOT_CONFORM になります — 正直ですが役に立ちません

### 主張しないこと

- **これは利便コピーであって、保存計画の代替ではありません**
- **記録の 6 項目のうち「取得記録との関係」は、実運用の大半で埋まりません。**
  外部取込由来の文書にしか無い値を読んでいるので、**通常の CMIS アップロードでは常に空**です。
  捏造はしませんが、「6 項目を記録する」と読まないでください
- **報告だけを見る読み手は、複製の digest を照合できません。** digest の入力には
  変換器や実行者も入りますが、それらは報告にも台帳項目にも載っていません

## 処分の一部を証跡に残すようになりました

- **cold move の MOVE モードでローカル本文を削除するとき、削除の前に「何を・いつ・
  どの規則で」を台帳に追記します。追記できなければ削除しません。** 取込とは逆向きの
  判断です — 取込は既に起きているので拒否できませんが、削除は**まだ起きていない**ので、
  拒否の代償は再試行だけです

### 主張しないこと

- **これは処分の全経路ではありません。** 証跡が付いているのは **cold move の MOVE モードに
  よるローカル削除 1 か所だけ**です。CMIS の通常削除・archive の物理削除・
  `lineage.retention.days` の purge はこの経路を通っていません
- **台帳に在ることは「その削除が正しかった」を意味しません。** 台帳が言うのは
  「この規則の下でこの削除が行われたと記録された」までで、**規則そのものの妥当性については
  何も言いません**
