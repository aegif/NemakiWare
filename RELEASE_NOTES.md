...
いません — 飛ばした行の ID はその WARN に出ます。**読めない行 1 行だけで定義されている
プロファイルは、この版の管理 API では消せません**: `?docId=` 付きの DELETE は分岐した対の
片方を消す操作で単独の行を 409 で拒み、ID なしの DELETE はその行を読めずに 503 になります。
CouchDB 側でその行を直すか消してください。行単位で「読めない」を答えるのは、ID 指定の
  ...
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  ...
  - **受信先のプロファイル行が解釈できないときも 503** です。従来は解釈できない行を
    ...
    **受信中のコネクタを名指ししている行があれば**そのコネクタへの配送を拒否します —
    ...
    この node の知らない名前を含む list は「除外」と確定できないので、この扱いにはなりません)。
    2 つの判定は欄ごとに独立です。コネクタの 2 欄が読める形でない行は**すべてのコネクタ**を
    名指すものとして扱います (壊れた行 1 つで全 webhook が止まる側に倒しています — その行
    宛てのイベントが「該当なし」として消える側より、直すまで止まる側を取りました) が、
    `allowedArchetypes` が読めて除外できるコネクタは止めません。逆に `allowedArchetypes` が
    読める形でない行は全 archetype 宛てとして扱いますが、コネクタの 2 欄が読めて名指して
    いないコネクタは止めません。
    `enabled` が `false` (リテラルでも文字列でも) の壊れた行は受信先になり得ないので
...
- **取込の自動解決 (`profileId` を省いた取込) が、`enabled: false` の行を解釈しなくなり
  ...
  **有効**な行が解釈できない場合は従来どおり 503 です (その行が既定だったかもしれない
  ため)。「無効」と読むのは JSON の `false` と文字列の `"false"` で、コネクタ側の一覧と
  同じ 2 形です (値が無い行は無効とは読みません)
