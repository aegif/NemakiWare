  ...
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  ...
  - **受信先のプロファイル行が解釈できないときも 503** です。従来は解釈できない行を
    ...
    **逆に、`enabled` の文字列で無効と読むのは mapper が読む形だけになりました** —
    `"false"` / `"False"` / `"FALSE"` の 3 綴り (前後の空白は除かれます)、および空文字と
    `"null"` (どちらも「値なし」として false)。`"fAlSe"` のようにそれ以外の綴りの行は、従来
    ...
    コネクタは止めません。
    `enabled` を false と読める壊れた行 (リテラル、文字列の `"false"`、明示的な null) は
    受信先になり得ないので数えません。行が直る (またはその値を読めるノードに切り替わる) まで続く状態なので、
