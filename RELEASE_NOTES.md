...
- **取込プロファイル (`/core/api/v1/admin/import-profiles`) も同じ扱いです。**
  ...
  **プロファイル**は確定的 ID の行も旧 ID しか無い行も索引を使わずに読めるので、多くの場合
  そもそも 200 が返ります。**コネクタの `GET` は違います** — 索引を使わない読みで確かめるのは
  「行が在るかどうか」だけで、行そのものは取り直しません。旧 ID しか無いコネクタは 404 では
  ...
  意味が薄いのと、管理 API 全部を索引不要にすると設定 DB の全走査が毎回入るため)。ただし
  **索引が答えられないときの `GET` は 409** です — 呼び出し元リポジトリの行がどれか索引に
  訊けず、行が 2 つあることだけが分かるためで、そこで 1 つ選ぶことはしません。
...
- **「読めなかった」が 500 や 400 で返っていた入口を 503 に揃えました。**
  DLQ (`/core/api/v1/ingest-dlq/...`)、取込 (`/core/api/v1/ingest/...`)、webhook の
  購読管理 (`/subscribe`) は、コネクタ・プロファイルの行が読めなかったときに Spring の
  **500** を返していました (「こちらの不具合」と読める答えですが、再試行で直る状態です)。
  これらも **503** になります。webhook 側の本文は理由を含みません — この入口は未認証で、
  理由の文面はその id に行が在るかどうかを明かすためです (ログには残ります)
- **IMAP IDLE の開始 (`POST /core/api/v1/ingest-scheduler/idle/start/{profileId}`) も
  同じです。** プロファイルやコネクタが読めなかったときの拒否は本文に「retry shortly」と
  書きながら **400** で返っていました。読めなかった場合は **503** になります (設定が本当に
  不正な場合は従来どおり 400)
- **チェックポイントの一括リセットが「全部消した」と答えなくなりました。**
  `DELETE /core/api/v1/ingest-scheduler/checkpoint/{profileId}` (scope 無し) が消せるのは、
  ...
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  ...
  - **コネクタ自身の解決も同じ扱い**です。コネクタの読み取りが答えられなかったとき
    ...
    **この対の拒否はセレクタが見せた対だけ**です。セレクタが失敗している間に確定的 ID の
    ...
    塞げますが、索引の再構築は規模によっては時間単位で続き、その間すべての webhook を止める
    と送信側の再試行期限を越えてイベントが落ちます。相方の secret で署名されたイベントは
