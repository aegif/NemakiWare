...
- **取込プロファイル (`/core/api/v1/admin/import-profiles`) も同じ扱いです。**
  索引再構築中に見えないプロファイル・コネクタへの `PUT` / `DELETE` は、従来の **404** では
  なく **503** を返します (存在しないものは従来どおり 404)。**プロファイルの `GET` は、索引が
...
- **取込で、コネクタが読めなかったときに「コネクタ指定なし」として別の取込フローに
  ...
  **コネクタが存在しないと確かめられた場合は従来どおり**推測に落ちます (存在しない
  ...
  指定した取込がすべて 503 になります。**委譲取込で読みが拒否された場合も監査に残ります**
...
- **`/fixity/scan/folder` に `status: "partial"` が増えました。** フォルダは在るが
  子の一部が読めなかった場合です。**そのときだけ** `unreadableChildren` に件数が出ます
  (読み切れたときはキー自体が出ません)。応答の `scope` / `scopeLimits` も変わり、
