...
- **webhook 受信 (`/core/api/v1/ingest-webhook/{connectorId}`) の受信先解決が索引を読まなく
  ...
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  ...
  - **受信先のプロファイル行が解釈できないときも 503** です。従来は解釈できない行を
    ...
    今は、飛ばした行の `defaultConnectorId` / `allowedConnectorIds` を生のまま読み、
    **受信中のコネクタを名指ししている行があれば**そのコネクタへの配送を拒否します —
    ...
    / `allowedArchetypes` を**本体と同じ読み方 (同じ mapper) で 1 欄ずつ読んで**決めます。
    数値は文字列として、`enabled` の明示的な null は false として、list の null 要素はその
    まま読み、mapper が読めない値 (知らない archetype 名、list でない `allowedArchetypes`
    など) だけを「読めない」とします (手書きの判定で mapper が読める値を「読めない」と扱い、
    確定できる除外を捨てていたのを直しました)。
    壊れた行の `allowedArchetypes` が読めて、そのコネクタの種類 (archetype) を含まない場合も
    宛先ではないので止めません (読める行なら受信側の archetype 判定で外れる行です)。
...
- **取込の自動解決 (`profileId` を省いた取込) が、`enabled: false` の行を解釈しなくなり
  ...
  **有効**な行が解釈できない場合は従来どおり 503 です (その行が既定だったかもしれない
  ...
  なので、一覧が `"42"` と呼ぶ行は件数にも数えられ、その ID で読めて消せます)
