...
- **取込プロファイル (`/core/api/v1/admin/import-profiles`) も同じ扱いです。**
  ...
  なく **503** を返します (存在しないものは従来どおり 404)。**`GET` はまず索引を使わずに
  呼び出し元の行を解決する**ので、多くの場合そもそも **200** です — 確定的 ID の行も、旧 ID
  しか無い行も、プロファイルなら索引に頼らず読めます。`GET` が 503 になるのは、行が在ると
  分かっていて読めなかった場合です。**コネクタの `GET` は少し違います** — 索引を使わない
  ...
  **索引が答えられないときの `GET` も、確定的 ID に呼び出し元の行があればそれを返して 200**
  ...
  無いとき** — 2 行とも旧 ID である、あるいは確定的 ID の行が別のリポジトリのものである場合
  で、そのときプロファイルは 409、コネクタは 503 です。
...
- **IMAP IDLE の開始・停止 (`POST /core/api/v1/admin/ingest-scheduler/idle/start/{profileId}`
  ...
  書きながら 400 という状態でした。読めなかった場合は **503**、同じ ID の定義行が 2 つある
  場合は **409** (再試行では直りません)、プロファイルが無いと確かめられた場合は **404**、
  設定が本当に不正な場合だけ従来どおり **400** です
