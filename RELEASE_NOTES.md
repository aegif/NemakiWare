...
- **DLQ の再実行と一覧・purge が、答えられなかったことを黙って埋めなくなりました**
  ...
  - **一覧の `nextOffset` は次のページがあるときだけ付きます。** また、**一覧は
    `(type, dlqId)` 順に並ぶようになりました** (`dlqId` は行ごとに一意で、行の一生を通じて
    変わりません)。従来は並び順を指定しておらず、Mango が選んだ索引の順でした — CouchDB
    3.3.3 の実測では `_id` 順でしたが、**どこにも保証されていない順序**だったため、
    `offset` は「未規定の並びの上の境界」で、前のページに出た行がもう一度出たり、
    どのページにも出ない行が生じたりしえました。**ページを見ている間にエントリを
    再実行・削除するとずれる**点は変わりません (その場合は一覧を取り直してください)
  - **並べられなかったページは、そうと分かるように返します。** `(type, dlqId)` の索引が
    まだ登録されていないデータベースでは、一覧は従来どおりの並びで返りますが、応答に
