  ...
  **200**) を返し、送信側はイベントを配送済みとして扱っていました (一覧が例外で失敗した
  ...
  - **コネクタ自身の解決も同じ扱い**です。コネクタの読み取りが答えられなかったとき
    ...
    **このコネクタ解決の答え (503 か否か) だけから未認証で分かること**を、答えが実際に
    ...
    はこの解決だけでは不在と区別しません。**セレクタが失敗している間だけ**、非 503 は
    「読める確定的 ID の行が在る」を意味します (不在はその間 503 になるため)。この最後の
    開示は、セレクタが失敗している間は一律 503 にすれば消せますが、その間すべての webhook を
    止めることになるので採りませんでした。署名が合わない要求と無効な行への要求にその間だけ
    401 でなく 503 を返せば、正しい送信側を止めずに POST 側のこの開示を消せます (GET の
    URL 検証は 404 側も同じ扱いにしなければ残ります) — 未対応の候補として記録しています。
    なお、受信側のプロトコル上のハンド
