...
echo ""
echo "NOTE: CouchDB credentials default to admin/password (matches the"
echo "      Docker dev container above). To override, append:"
echo "      mvn jetty:run -f core/pom.xml -Pdevelopment \\"
echo "        -Ddb.couchdb.auth.username=<user> -Ddb.couchdb.auth.password=<pwd>"
echo ""
