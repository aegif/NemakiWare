...
public class QueryTckRunner extends AbstractRunner {
    
    private static final String PARAMETERS_FILE = "cmis-tck-parameters-docker.properties";
    
    public QueryTckRunner() throws Exception {
    ...
    public static void main(String[] args) throws Exception {
        System.out.println("Starting NemakiWare Query TCK Test Execution...");
        
        QueryTckRunner runner = new QueryTckRunner();
        ...
        System.out.println("Running Query TCK tests with " + groups.size() + " test group...");
        ...
        try {
            ...
            System.err.println("Query test execution failed: " + e.getMessage());
            e.printStackTrace();
            System.exit(1);
        }
        ...
        try (PrintWriter textWriter = new PrintWriter(new FileWriter(new File(reportsDir, "tck-query-report.txt")))) {
        ...
        try (PrintWriter xmlWriter = new PrintWriter(new FileWriter(new File(reportsDir, "tck-query-report.xml")))) {
            xmlReport.createReport(runner.getParameters(), groups, xmlWriter);
        }
        
        System.out.println("\n=========== Query TCK Test Summary ===========");
        int totalTests = 0;
        int totalFailures = 0;
        int totalWarnings = 0;
        ...
        System.out.println("\nQuery TCK test execution completed!");
