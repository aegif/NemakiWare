...
37. **Authentication Context Provider 包括的ドキュメント化** ✅
  ...
  - **10個の重要デザイン決定**（行番号付き詳細）:
    ...
    7. Logout Redirect Behavior (Lines 85-86): redirects to /core/ui/index.html after logout、rationale full page reload clears all React state、implementation window.location.href assignment、advantage clean slate no stale state from previous session、alternative considered React Router navigate rejected doesn't clear state
...
38. **Application Entry Point 包括的ドキュメント化** ✅
  ...
  - **10個の重要デザイン決定**（行番号付き詳細）:
    ...
    6. Root Path Redirect Pattern (Lines 40-41): both / and /index.html redirect to /documents、rationale default landing page after login、implementation <Route path="/" element={<Navigate to="/documents" replace />} />、advantage consistent entry point replaces history for clean back button、HashRouter URL http://localhost:8080/core/ui/#/
...
**症状**: `http://localhost:8080/core/ui/index.html`が404を返す
