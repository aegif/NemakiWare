...
- 2026-09-15 / バッチ 2（R23）: 新しい DLQ 行を確定 `_id` に（CAS なし）。錠 2 本、control ZV2。実測: ZV2 FIRED、全ユニット 6,985 green。レビューは Codex / subagent とも CONVERGED。subagent の条件「tombstone 上の `_rev` 無し再作成は 409 か」は scratch の CouchDB 3.3.3 で POST / PUT とも 201 を実測して閉じた（リポジトリ内の purgeTombstone の腕はその逆を仮定した未計測の防御で、開いていない）。P3: runner の helper が既存コメントを奪っていた（直した）、凍結領域の 429 文言（→ R42）
- 2026-09-15 / バッチ 3（R1 = CAS。payload 凍結を CAS のためだけに限定解除）: `upsertDlqCas`（読んだ _rev への条件付け + transient bookkeeping）、保存は最大 3 回再マージ、予約は競合で 429。錠 6 本、control AA3〜AE3。実測 9/9 発火、全ユニット 6,991 green。レビューは Codex / subagent とも CONVERGED。R24 は閉じた（`== null` 判定は事実になり、旧名を名指すコメント 7 か所を書き直し）、R35 は残る（token 等値の所有権検査は CAS で変わらない）、R42 は窓が消えて閉じた。新規 R43（確定書き込みの fallback 腕が死んでいる、凍結領域）。P3 で直したもの: RELEASE_NOTES の「予約・確定も再マージ」を事実に、§5 の本数 656、gate テストの古いコメント。記録のみ: selector 不応答で document API が応答する場合、差分前は何も書けなかったが今は確定 id の create が通り「履歴不明」行（failureCount=0・assumed・新 token）が書かれる（fail-closed 方向）。ERROR「its row kept changing under it」は selector 不応答時の create 競合にも出る（文言）
- 2026-09-15 / バッチ 4（R2）: /tmp/batch2 の R2 案（`getOrRefuse` の中で walk）は署名検証より前の未認証全走査を戻すもので `dece81f7d` の判定と衝突するため採らず、走査を署名検証・レート制限の後（`refuseUnlessUniquelyDefined`）に置いた。製品 3 ファイル +75/−2、錠 11 本、control 8 本（AF3 / AG3 / AJ3〜AO3。AH3・AI3 は使用済み）。実測: 9/9 発火（R24 のコメント書き直しで錨が外れた VQ2 の再錨を含む）、全ユニット 7,002 green。確認レビュー: Codex CONVERGED、並行レビューも CONVERGED（subagent の 1 本はクレジット切れで中断）
- 2026-09-16 / バッチ 5（R3）: セレクタ障害の窓で 401 と 503 が「読める行が在るか」を分けていた。読みが `Resolution(connector, selectorAnswered)` を返し、窓では refusable な答えを 1 つにまとめる（本文まで同一。錠は本文の一致を実測値どうしの比較で表明する）。一律拒否は採らず、正しい secret の送信側は通る（錠 `theRightSecretIsStillDispatchedWhileTheSelectorIsDown` / `theHandshakeStillEchoesWhileTheSelectorIsDown`）。製品 3 ファイル +106/−21、錠 11 本、control 9 本（AP3〜AX3）+ 再錨 4 本（XD / XE / AN3 / AO3）= 13/13 発火、巻き添えの宣言を 3 本補って再実測 3/3、全ユニット 7,013 green。レビューは Codex / subagent とも CONVERGED。subagent の P3 3 件を処置: RELEASE_NOTES の新 bullet が入れ子の途中に入って以降のサブ項目を付け替えていた（移動して文末の「。」を戻した）、「未認証では読める行が在るか分からない」が握手を除外していなかった（文面と javadoc を限定）、この行に実測が無かった（この行）
- 2026-09-15 / バッチ 6（R4）: 取込後のリンクが認可を**一度も訊かずに**作られていた。入口に authorizing profile と request を足し、`createLinkAuthorized` を抽出して取込中のリンクと同じ核を通す（答え方は `outsideAnImport` のまま）。`FetchSupport` は材料が無ければ拒否。製品 6 ファイル +67/−15、錠 6 本、control 5 本（AY3 / AZ3 / BF3 / BG3 / BH3）+ 再錨 2 本（VV / VW）= 7/7 発火、巻き添えの宣言を 2 本補って再実測 1/1、全ユニット 7,018 green。オーケストレータ 3 本（Slack / Teams / Mattermost）の呼び出し側は**錠で測っていない**（その 3 クラスにユニットテストが無い）。レビューは Codex / subagent とも CONVERGED。subagent の P2 を処置: 8 引数 `createDirectRelationship` がこのバッチで本番の呼び出し元を失い、テスト 3 本だけの扉になっていたので削除し、3 本を `...AuthorizedForTest` に付け替えた（同ファイルは 5 / 6 引数版を同じ理由で消した先例を持つ）。P3 5 件も処置: 消えた入口を指す javadoc とテストのコメント、インターフェースの「取込中と完全に同じ」（読み直すのは ACL とコネクタ行だけ）、正典の「渡し漏れは拒否として現れる」（両方落としたときだけ真）。残件 R44 / R45 を記録。宣言漏れは読解の予測 2 件（VT / UF2）に加えて実測で 3 件出た
- 2026-09-16 / バッチ 7（R33）: multipart の入口が本文の解析と取込を 1 つの catch で囲っていたため、取込の失敗まで 400「Invalid request」になっていた（JSON 入口は 500）。解析だけを残して取込を外に出し、型付き 3 種の rethrow を削除。QR2 は錨が消えたので「型付きだけを包む」サボタージュに張り直し、BI3（全部を包む）と測る主張を分けた。錠 4 本、control 4 本 + 再錨 1 本 = 5/5 発火、全ユニット 7,022 green。レビューは Codex / subagent とも CONVERGED。**subagent が実測で、この処置が作った過剰投げを 1 件見つけた**: JSON リテラル `null` は Jackson が投げずに null を返すので、狭めた catch を素通りして取込で NPE → 500 になっていた（JSON 入口は同じ入力に 400）。400 の腕を足して錠 BL3 で測った。ほかに P3 2 件を処置（旧実装を現在形で説明していた既存錠のコメント、両扉比較の錠が message しか見ていなかった点）。残件 R46 を記録
- 2026-09-16 / バッチ 8（R25）: `classifyErrorStatus` の 500 fallback に落ちていた拒否のうち **2 つ**を 409 / 503 に分けた（重複判定の列挙が答えなかった / 不完全だった）。錠は**製品が作ったメッセージを製品の分類器に通す**形（実 execute から取る）。錠 3 本、control 3 本 + 再錨 2 本（TO / UE は 409 の条件式を広げたので先頭だけを指すように）= 5/5 発火、全ユニット 7,025 green。**3 つ目「自動解決の曖昧さ」は取り下げた**: subagent のレビューが「腕が到達不能」と指摘し、呼び出し経路を自分で辿って確認した（メッセージを作るのは `executeWithAutoResolve` だけ、その本番呼び出し元は `CloudDriveResource` 1 つで、そこは分類器を呼ばず 200 で答える）。**取り下げたのは腕と錠と control と、4 か所に書いた「409 を返すようになった」という主張**。残件 R25 の元の文（60 巡 P3）の前提そのものが、この半分については偽だった。Codex は CONVERGED と判定していたので、到達性は 2 本のレビューのうち 1 本しか見ていない
- 2026-09-16 / バッチ 9（R5）: 製品は変えていない。書き込み地点を数え、chat と note の入口を通す実測の錠 2 本と構造の錠 1 本を足した。**初版の主張は 2 点とも偽で、レビューが P1 / P2 として出した**: (a)「全部数えた」は 1 ファイルしか数えておらず `IngestMetadataService` の 4 か所が漏れていた、(b)「入口は自分の書き込みを持たない」は偽で、**4 入口とも `execute` が返った後に属性を書く**（初版はそのうち chat の 1 件だけを R47 に書き、しかも「3 回目の問い直しは窓を縮めるだけ」と書いた。これは `createLink` が同じ形を「避けられる読み」と呼んで再問い合わせを入れた先例と逆向き）。さらに構造の錠は**公開ラッパ**（中身は捕捉スコープと try だけ）を読んでいて、入口のコードを一行も見ていなかった。処置: 錠を `…Internal` に向け、探索語を 5 種に広げ、2 ファイルの書き込み本数（9 と 4）を数える錠にした。R5 の行は範囲付き（`CanonicalImportServiceImpl` の 9 か所、作成・checkIn・関係は execute / createLink の中）に直し、R47 を 4 入口ぶんに広げて「不可避ではない」と書き直した。錠 3 本、control BQ3 / BR3 = 2/2 発火、全ユニット 7,028 green
- 2026-09-16 / バッチ 10（R47）: `execute` が返った後の装飾 5 か所に 3 回目の問い直しを入れた（`refuseDecorationIfNoLongerAuthorized`）。拒否は警告で、取込は失敗にしない（`createLink` の先例に合わせた）。製品 +89/−5。錠 3 本は「認可の応答回数」で窓に正確に着地させる（`execute` の 2 回には yes、3 回目に no）。VF は新ヘルパーが同じ 2 行で始まるため錨が 2 か所に当たるようになり、後続行まで含めて張り直した。**確認レビュー 2 本が NOT CONVERGED**。(a) 先例 `createLinkAuthorized` から解決だけを写して「コネクタが読めなければ拒否」の腕を写しておらず、先例が閉じた fail-open を再生産していた（両者一致の P1）。(b) 本数の錠が問い直しの出現数しか数えておらず、**問い直しを持たない新しい扉を足しても緑**だった（コメントはそれを捕まえると書いていたので偽）。(c) 錠が「認可の応答回数」で窓に着地するため、回数を変える古い control 7 本の宣言が漏れていた。(d) コストの記載がコネクタ読みと ACL 評価のぶん過小。処置: コネクタ不達の拒否と錠 BV3、metadata service 使用箇所数の錠と BW3、6 control の宣言を実測で補完、コネクタ読みを委譲判定の後ろへ移してコストを事実に。製品 +109/−5（うちコード 72 行）、control 11/11 発火、全ユニット 7,032 green。R44 / R45 は残る（1 行ずつ追記。初版は表の 4 列目より後ろに書いてしまい捨てられる位置だったので中に入れ直した）。
  **2 巡目のレビューでまた P1**: metadata service の本数の錠について「新しい扉を捕まえる」と書いたが、`applyCaptureWindow` は metadata service を通さず `contentService.update` を直接呼ぶので、同じ形の扉は両方の数を動かさない。1 巡目 (b) と**同じ形の過大主張**を、狭い範囲でもう一度書いていた。ユーザー指示「同じ領域で 2 度目の P1 が出たら残件に戻して止まる」に従い、文面を事実に直して穴を R48 として記録し、ここで止めた（3 つ目の数は足していない）
- 2026-09-16 / バッチ 11（R48、錠だけ・製品不変）: 3 つ目の数 `contentService.update(` = 5 と control BX3 を足し、実測では発火した（BX3 は R5 の書き込み本数の錠も落とすので宣言済み）。**が、錠のコメントの「捕まえないもの」の列挙が2 巡続けて過大主張と判定された**（1 巡目: 「その形はここでしか見えない」が偽 — R5 の錠も見る。2 巡目: 「checkIn の properties に載せる装飾は R5 の錠が見る」が偽 — 呼び出し地点が増えないのでどちらも見ない。`checkOut` / `deleteObject` はどの錠も数えていない）。指示 §3「過大主張が 2 度目なら 3 つ目の数を足さず残件のまま進め」に従い、数と BX3 を取り下げた。製品は最初から触っていない。既存コメントの「5 つのうち 4 つが metadata service 経由」は事実でない（5 つとも経由し、chat が直接書きを**追加で**持つ）ので、その 1 文だけ直した。正典 §5 の本数がバッチ 4 以降 664 のまま止まっていたのを 692 に更新
- 2026-09-17 / Phase B（通し 3 回目、製品不変）: 692 本を 17 時間 18 分で完走。**692/692 発火、不発 0**、exit 1 の理由は宣言漏れだけ（6 control・のべ 7 本・異なり 4 本。いずれも R2 / R3 の錠で、2 回目の通しより後に生まれたため宣言が測られる機会がなかった）。ログ末尾の節から補完して RD / TX / VX / WC / WM / XN を再実測 6/6。走り出しは 254 本目で落ちている: RD の細工が `read()` の戻り値（R3 で `Resolution`）とかみ合わずコンパイルできなくなっており、**錨の事前検査は錨の一致しか見ないので通り抜けた**。前回完走以降に変わった製品ファイルを狙う control 207 本を `--compile-check` に掛け、壊れていたのは RD だけと確かめてからやり直した。
- 2026-09-17 / バッチ 12（R46）: multipart 入口で保存済み一時ファイルを開く読みだけを自分の try に分け、IOException を 400「Invalid request」から 503 に外した（他の 3 つの 400 は経路も文言も不変）。解析用 catch に IOException の腕を足す形は採っていない — mapper が検査例外を投げる形になれば壊れた本文まで 503 になり逆向きの過大投げになる。到達性は web.xml の `file-size-threshold` 1MB と `getInputStream()` の検査例外で繋いだ。錠 2 本、control BY3 / BZ3（2/2 発火）。RELEASE_NOTES の「一時ファイルの読み取り失敗も 400 のまま」はこの処置で偽になるので同時に直した。**確認レビュー 2 名とも CONVERGED**。P3 を 1 件取り込み（過大投げの番人が「取込に届く」を `execute` が呼ばれたことで代用していたので、ArgumentCaptor で開いたストリーム・filename・mimeType を見るようにした）。もう 1 件の P3（503 本文の `getMessage()` が一時ファイルのパスを認証済み未認可の利用者に渡しうる。既存のクラス handler も同じ形）は残件 R49 に 1 行。
- 2026-09-17 / バッチ 13（R13）: DLQ 一覧の `offset` は「未規定の並びの上の境界」だった。使い捨ての CouchDB 3.3.3 で実測してから設計した — 索引が無いと `sort` は 400 `no_usable_index`／本番と同じ 7 索引を置いても sort 無しの並びは `_all_docs`（`_id`）順で、その run では安定だがどこにも保証がない。並びを `(type, dlqId, _id)` にし、索引 `idx_type_dlqId_id` を `Patch_IngestMangoIndexes` に 1 本追加。索引が無いデータベースでは 400 を吸って従来の並びで返しつつ応答に `stableOrder: false` を付ける（一覧ごと拒否しない — 各行は喪失の唯一の記録）。**初版は `(type, dlqId)` の 2 キーで、レビュー 2 名が独立に「全順序ではない」と指摘（Codex P1 / subagent P3。同じ `dlqId` の双子行が在りうることは `deleteDlqEntry` の javadoc が既に扱っていた）** → `_id` を足して実測し直した（双子 4 組を含む 12 行で重複 0・取りこぼし 0）。作業中に自分の欠陥を 2 つ測定に捕まえられた: 新メソッドが「文書一覧が無い応答」の判定を複製して事前検査が錨の重複で検出（判定を 1 か所に戻した）、拒否の錠が守りを外しても緑（control CC3 が不発。両方の読みを失敗させていたため fallback 側の拒否が同じ assertion を満たしていた）。subagent の P3-2（印の文面が 400 の支える以上を断定し、勧める `limit` が上限 500 で実行できない）も処置。P3-3（印を捨てる公開オーバーロード）は残件 R50。錠 5 本、control CA3〜CE3、実測は都度発火。
- 2026-09-18 / バッチ 14（R51、リリースゲートの発見）: Playwright の `Archive Management API › should list archives` が 500 を返していた。実機で追うと「145 行が読めない」で、原因は共有 mapper の非対称だった — `GregorianCalendar` を epoch ミリ秒で書き CouchDB は**整数**で保存するのに、Cloudant SDK が `Map<String, Object>` で返すとき JSON 数値を `Double` に広げるため、再直列化した `1.786536650704E12` を Jackson 3 のカレンダー復号が受けない。**3.4 の退行ではなく、3.4 が入れた拒否が露出させた既存欠陥**（それ以前は「ゴミ箱が空」に見えていた。RELEASE_NOTES 886 行の記述と一致）。`DaoHelper` に deserializer を 1 つ足して解決。実機で 500 → 100 件。**途中で誤った修正を 1 つ書いて実測で否定した**: 姉妹メソッドのコメントを根拠に「view の `getValue()` ではなく `getDoc()` を読む」形にしたが、fixture を実物どおり（`Double` の `archivedAt`）にすると doc 経由でも落ちたため戻した。錠 4 本（広げられた値・整数・不在・端数）、control CH3 / CI3。CH3 は初版で「自分の assertion で落ちていない」と判定され、`assertDoesNotThrow` で包んで発火させた。
- 2026-09-19 / 通し 4 回目（704 本、7 時間 8 分、**exit 0**）: **704/704 発火、不発 0、宣言漏れ 0、錨外れ 0、「復元後に緑でない」0。** このブランチで初めて何も直すところが無く終わった通し。前の 2 度の中断はどちらも細工でも製品でもなかった: RD の細工が製品の型変更とかみ合わなかった件（3 回目の冒頭、`--compile-check` で 207 本を確かめて再開）と、13 時間走の **601 回目の `npm install` が一過性に失敗**した件（600 回は成功）。後者を受けて runner の maven 呼び出しに `-Dskip.npm` / `-Dskip.installnodenpm` を入れた — frontend-maven-plugin が毎ビルド npm を走らせており、小さなクラス 1 本のビルド 43 秒のうち約 40 秒・通し 1 回で約 7.8 時間を占め、しかも control ごとに npm レジストリへ手を伸ばしていた。UI バンドルはどの control も測っていない。**フラグの有無で全ユニット 7,046 本 green が完全一致することを実測してから入れた。** 通しは 17 時間 → 7 時間になった。この smoke test で宣言漏れも 1 件見つかっている（P3-4 で後から足した錠が CA3 でも落ちる）。
