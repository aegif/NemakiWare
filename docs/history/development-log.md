# NemakiWare 開発ログ (per-commit 詳細)

このファイルは 2026-07-26 まで `CLAUDE.md` 内に置かれていた開発履歴を、
**一字一句そのまま**移したものです。内容の削除・要約は行っていません。

移した理由: `CLAUDE.md` は全セッションで常時ロードされるため、履歴を同居させると
毎ターン 8 万トークン超を消費していました。履歴は「必要になったときに読む」
参照資料であり、常時展開すべき運用知識ではありません。

- **利用者向けの変更履歴** → [`RELEASE_NOTES.md`](../../RELEASE_NOTES.md)
- **ACL-epoch の設計と実装進捗** → [`docs/design/acl-epoch-fencing.md`](../design/acl-epoch-fencing.md)
- **リポジトリ固有の落とし穴** → [`CLAUDE.md`](../../CLAUDE.md)
...
再現した 5 件を再デプロイ後に流し直し: **39 passed / 2 failed / 3 flaky**。
ユーザー一覧 API の 3 件 (一覧 / HATEOAS links / ページング) は解消した。

**残る 2 件は製品の欠陥ではない** (`group-management-crud` の 2 件)。
このリポジトリには**グループが 2,989 件**溜まっている (1 週間分のテストの滓。
うち 1,843 件が `c17082x55g21` 系の規模テスト由来、98 件は実在の
`cloud-google:` ディレクトリ同期)。UI 自体は 20 件/ページで正しくページングして
おり、グループ一覧 API も 200 件で 3.2 秒と平坦。落ちているのは
**テストが 2,989 件の中から特定のグループを UI 経由で探す**ところ。

→ **掃除するかはオーナー判断**とした。実在のディレクトリ同期データが混ざって
いる共有 dev 環境で 2,900 件を消すのは取り返しがつかない。

### 内訳 (21 件)

| 分類 | 件数 | 根拠 |
|---|---|---|
| 負荷・干渉由来 | 16 | 静かなスタックで単独実行すると通る |
| 製品バグ (ユーザー一覧 N+1) | 3 | 単独でも再現。**修正済み**、解消を実測 |
| テストデータ蓄積 (2,989 グループ) | 2 | UI・API とも正しくページングしている |

(初稿は 16 + 5 + 2 = 23 と書いていて**合計が 21 に合っていなかった**。
単独で再現した 5 件の内訳が N+1 3 件 + データ蓄積 2 件である。2026-08-25 訂正)
...
# 付録: セキュリティ対応履歴 (旧 CLAUDE.md `## セキュリティステータス`)

これも常時ロードから外した履歴です。個々の項目は「対応済み」の記録であり、
日々の作業で参照する運用知識ではありません。以下、一字一句そのまま。
