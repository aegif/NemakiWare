...
> **前提の片方だけ満たした (2026-08-25)。** この表が V2 の条件に挙げていた
> 「`create()` の失敗を失敗として扱う」は**実施済み** —
> `CloudantClientWrapper.create()` は**両オーバーロードとも**起動後は throw するように
> なった (初稿は `create(Map)` だけを直して「create() が」と書いており、**過大だった** —
> 2026-08-25 のレビューで訂正)
> (roadmap 2-1 の残り。それまで「これは初期起動では正常」というコメント付きで
> 全例外を握って null を返しており、**まさにこの V2 の検証 GET だけが
> それを捕まえていた**)。
>
> **それでも V2 は落とさない。** もう片方の条件
> 「Cloudant の read-your-writes をやめる判断」が残っている。単一ノードの
> CouchDB なら書いた直後に読めるが、クラスタ構成では quorum の取り方次第で
> 読めない瞬間がありうる。**これは配備形態の決定**であって、
> ここで勝手に前提にしてよいものではない。オーナー判断待ちとして残す。
