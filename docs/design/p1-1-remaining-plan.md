...
| D-4 ✅ 2026-08-24 | **(d)-5: 会話の範囲 (window/scope/reason) の恒久的置き場** — 解消: 置き場は Process 属性**ではなく** journalFacts (event の digest 被覆区画。INTERNAL_ONLY はカタログへ構造的に出ない — `LineageRecord.fromV2` が projection しない)。予定どおり §4 の flip 前提と同じ設計で決めた | `CaptureEvidenceField:226-240` 全て `V2Home.none` — flip で行き場を失う | 済 ((e) Step 2 — §4 の 2 行目)。恒久 none は participants / channelName のみ |
...
| ✅ 2026-08-24 **CatalogSecretBoundary 統合** — 決定: qualifiedName / fullyQualifiedName を identity 免除 (scheme+path 可・`?`/`#`/userinfo 不可) に加える。**http(s) は identity 全 6 名に対し拒否** — 大小文字非依存・文字列中どこでも (`(?i)\bhttps?://`。sharing link = §4 の脅威は token-in-path の http(s) で、query/fragment 検査では見えない。正当な sink identity に http(s) は無い)。ネスト map の leaf は**traversal が取った key** (文字列分割は `x.name` に免除を渡す)。**3 sink (Purview/Atlas/Dataplex) とも sealed() を通す** — Atlas は**参照を足した後**に seal (順序が逆だと定数 3 個しか見ない) | disclosure §4、敵対レビュー finding 1/2、オーナーレビュー | boundary 29 tests + sink 契約 21 + unwire/revert 実測 |
