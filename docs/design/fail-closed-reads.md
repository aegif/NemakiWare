...
| R51 | **リリースゲートの発見（製品欠陥、このブランチの範囲外）**: ゴミ箱（アーカイブ）一覧が、アーカイブのある DB では常に 500。原因は `CouchArchive` の Calendar 項目（`archivedAt` / `coldArchivedAt` / `created` / `modified`）が **JSON の浮動小数点数**として渡り、この木の Jackson 3（`DaoHelper.createConfiguredObjectMapper`）が `GregorianCalendar` に変換できないこと。実測: 同じ値を `Long` で与えると復号でき、`Double` だと `Cannot deserialize ... from Floating-point value (reference chain CouchArchive["archivedAt"])` で落ちる。**view の `getValue()` を `getDoc()` に替えても直らない**（両方 Double で届く。一度その修正を書いて実測で否定した）。3.4 の拒否（258b13bc3）が入る前は同じ条件で「ゴミ箱が空」に見えていたので、**退行ではなく既存欠陥の露出**（RELEASE_NOTES 886 行の記述と一致）。nb33 実機: `bedroom_closet` 716,821 行、一覧は「145 行が読めない」で 500。Playwright の `Archive Management API › should list archives with admin credentials` がこれを捕まえている | 共有 mapper の変更になるため独立したバッチとレビューが要る | |
