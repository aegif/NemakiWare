...
| R46 | multipart 入口で `content.getInputStream()` の IOException は今も 400「Invalid request」。アップロードされた一時ファイルの読みはサーバ側の理由で失敗しうるので、読みが確かめていないことを「あなたのリクエストが不正」と断定する形が狭めた try の中に残っている（バッチ 7 の subagent、記録のみ。この差分が作ったものではない） | 型分け | |
| R47 | **4 つの archetype 入口はすべて `execute` が返った後に属性を書く**（mail: `fillMissingMessageMetadata`、note: `fillMissingNoteMetadata` と dedupe skip 時の `applyNoteMetadata`、record / chat: `fillMissingArchetypeMetadata`、chat: `applyCaptureWindow`）。実体は `IngestMetadataService` の 4 か所と `CanonicalImportServiceImpl` の `applyCaptureWindow` で、いずれも 2 回目の問い直しより後の `getContent` → `update` の読み書き対。**「不可避の一瞬」ではない**: `createLink` は同じ形を「最後の認可の後に置かれた**避けられる** DB 読み」と呼んで再問い合わせを入れた先例を持つ（バッチ 9 の subagent P1。初版は chat 1 件だけを「窓を縮めるだけ」と書いていたが、数え漏れと先例との矛盾を指摘されて広げた） | 入口の post-execute 装飾に 3 回目の問い直しを入れる（別バッチ） | |
...
- コントロール 664（2026-09-16 時点。内訳: 通し前 621 → 不発の WX を退役して 620 → Phase C で
  25 新設 = 645 → バッチ 1〜3 で 11 新設 = 656 → バッチ 4 で 8 新設）。**通し negative-control は
  ...
  **2 回目 2026-09-15〜16（664 本、15 時間 23 分、exit 1）: 663 発火、不発 1（SN2 — 宣言していた
  ...
  どちらも宣言はログ末尾の節から機械的に補完し、ID 指定で再実測（68/68、10/10）。
