...
**証拠の削除方針はオーナー判断**なので、ここでは決めない。選択肢と対価だけ置く。

| | 意味 | 対価 |
|---|---|---|
| 無期限 | 取込 1 件 = 1 行が永久に残る | `nemaki_lineage` が取込量に比例して単調増加する。10 万件取込で 10 万行 |
| 有限 | `capturedAtMs` を起点に purge | **「いつ取り込んだか」の証拠が消える**。lineage イベント本体は別途 90 日で消えるので、期限を揃えないと片方だけ残る |
