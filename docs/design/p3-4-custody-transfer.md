...
- **P2 (Codex) — webhook のコネクタ解決で、セレクタが失敗したときの対の検査が消える。**
  ...
  落ちる。相方の secret で署名された要求はやはり検証に落ち、その間の管理 API の書き込みは
...
- **P3 (Codex) — 食い違う対で「どれも触りません」が偽だった。** 走査は正規化対象の行を
  **食い違いが分かる前に**集めるため、後段の正規化パスが食い違う対の確定的 ID 行を書き換えていた。
  ...
  行を書き換えないためで、失うものは無い (対が立っている間は書き込み動詞がどのみち 409、
...
`jp.aegif.nemaki.rest.ingest.**` と `CloudantClientWrapperViewValueTest` の **99 クラス 1084 本が green**。
