...
- **P2 (サブ) — 未認証リクエスト 1 本ごとの `nemaki_conf` 全走査。** 1 巡目で足した
  ...
  **取り下げた。** 正しい形は「走査しない」: `get()` が「両方の読みが不在と答えた」と
  ...
  現在の記述は 9 巡目・10 巡目の節**) (3 巡目が「消える」を
...
- **P2 (サブ) — セレクタ障害の窓での存在開示。** (失敗, 健全な確定的行) → 401 (署名検証へ)、
  ...
  正しい送信側を止めずに窓の開示を消す) も 8 巡目が挙げ、未対応の候補として記録した。**
...
- **P3 (サブ) — 第 3 の選択肢**: 窓の間だけ署名不一致・無効の要求に 503 を返せば、正しい
...
コミット `c8449112e` に対して。P1 なし。サブエージェントは初めて `CONVERGED`。Codex の P2 は
...
- **P2 (Codex) — 非 503 の「読める行が 1 つ在る」は確立した事実より強い。** セレクタが pair の
  ...
  穴)。「読める行が少なくとも 1 つ見つかり、2 つ目は見えなかった」に 4 か所とも直した
  (サブ P3-4c と同じ指摘)。
...
型分けが外れていると P2 を出した。HEAD と復元後の木では wrap は健在で、読まれたのは control
**QI** — まさにその wrap を外す細工 — の適用中の木だった。通しとレビューを重ねない理由が
そのまま出た。)
