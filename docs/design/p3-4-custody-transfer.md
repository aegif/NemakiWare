...
- **P2 (サブ) — `(` を見たら 2.x と決め打ちしたため、この木に実在する 3.x の形を黙って捨てて
  ...
  (`@TempDir Path` など)。`ExportsRefuseMissingBytesTest` は 24 本中 16 本がその形で、
  **このクラスを使うコントロール 11 本**で宣言漏れ検出が構造的に空になっていた。実物のレポート
  (`testcase name="…(Path)"` / `…(String)[1]`) で確認。**盲点はこのクラスの 24 本中 16 本の
  引数付きメソッド**で、無引数の 8 本は旧 parser でも読めていた (このクラスを使うコントロールは
  11 本だが、「11 本で検出が空だった」という当初の書き方は言い過ぎだった)。→ **括弧の前に
...
**→ 27 巡目に 2 点訂正**: 「集計行の除外」は当時も到達しないガードで (アンカー付き照合が先に
弾く)、赤くなったのは別の理由。「出口」3 本は**判定関数の戻り値だけ**を測っており、`main()` の
`sys.exit` の配線は測っていなかった。
...
**正のコントロール** (どのアブレーションでも赤くならない — 両パターンの構造がその行を弾くため)
と明記した。入れ子クラスの case も同じく正のコントロール。
