...
(見出しの件数は本文の列挙に合わせてある。Codex の P3 は「綴り錠が 3 綴りのうち `"FALSE"` を
...
- **P2 (サブ) — 新設した self-test 4 本のうち 1 本が、自分の名指す保護を外しても緑。**
  ...
  **クラス名**を返し、`failed_as_assertion` が後続 stanza まで走査するため「クラス名という錠が
  ...
  self-test は 3 本追加 (23 → 27) し、**各保護を外すと 1 本ずつ赤くなることを実測**。
...
- **残余リスク (サブ)**: この 2 クラスを使うコントロールは **176 本**あり、未宣言を実測したのは
  9 本だけ。手導出が 4 巡連続で漏らした事実は残り 167 本にも等しく当てはまる。**次の通しは
  初回ほぼ確実に非 0 で終わる**見込み (fatal がそれを露出する)。
