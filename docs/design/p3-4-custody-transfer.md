...
**`codex:codex-rescue` は Codex CLI をリポジトリへの書き込み権限つきで動かす。**「REVIEW ONLY」は
プロンプト上の指示にすぎず、サンドボックスではない。`general-purpose` サブエージェントも
全ツールを持つ。21 巡目のあと、私が読むだけだった時間帯に本番ソースとテストが書き換わり、
**通しが 2 度とも「実行中にファイルが変わった」で停止した** (155/322 と 3/167)。ランナーの
並行編集ガードは正しく働き、細工を残さず止めた。

私は最初これを「原因不明の書き込み」と報告した。**可能性を潰せていないのに潰したように書いた** —
このバッチの主題をまた手順で踏んでいる。変更内容は Codex 21 巡目の P1 と一対一で対応する。
...
- **10,000 件の上限が、部分的な削除計画を「完成した計画」として返していた。** 上限を超えた
  ...
  「進まないページ」も**例外**にして、呼び出し側が警告し、部分削除をしない形にした。
  文面も「一覧できなかったので古いエッジが残る」と、破った約束を名指しする。
- **削除に失敗した関係が capture 証跡に FAILED として記録されなくなっていた。** 分離で
  落とした 1 行を戻した。

**開いたまま残す 1 件**: `createDirectRelationship` の中の「関係が既にあるか」の読みは
書き込みを決める読みだが、認可を問い直していない。閉じるには profile・connector・
対象フォルダを **2 クラス 10 か所**の呼び出しに通す必要があり、この巡で安全に入る範囲を
超える。**半端に入れず、開いていると名指しして残す** — コードのその場所と、ここに書いた。
...
**これで、このバッチの主張はすべて測定に裏打ちされた**: 負のコントロール 370/370、
単体スイート 6720 本 Failures 0。
