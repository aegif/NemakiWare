...
1 回目の測定で **UM2・UN2・UR2 が「発火せず」**— 3 本とも錠がサービスをスタブしており、
サービスを壊しても緑だった。**「helper でなく呼び出し側を壊せ」の裏返し**である。
UM2/UN2 はコントローラ側に張り替え、サービス側は実サービスを叩く錠を新設して UU2/UV2 に
分けた。UR2 は錠自体を作り直した。**再測定で 25 本すべて発火**、宣言漏れ 3 本を実測で補った。
**全ユニット 6888 本 green** (6876 → 6888)。
