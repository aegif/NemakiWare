...
### 25 巡目 (Codex + サブエージェント、並行) — P2 5 (Codex 3・サブ 2、重複なし)・P3 8、`NOT CONVERGED`
...
- **P2 (サブ) — `(` を見たら 2.x と決め打ちしたため、この木に実在する 3.x の形を黙って捨てて
  ...
  **このクラスを使うコントロール 11 本**で宣言漏れ検出が構造的に空になっていた。実物のレポート
  (`testcase name="…(Path)"` / `…(String)[1]`) で確認。**盲点は 24 本中 16 本の引数付き
  メソッドで、無引数の 8 本は旧 parser でも読めていた** (「11 本で空になっていた」は言い過ぎ
  だったので 26 巡目で訂正)。→ **括弧の前にドットがあるかで 2.x と 3.x を分ける**。self-test は**この木の実物の 2 形**に差し替えた (旧 case の `someLock[1]` は
