...
**VQ を取り下げた。** 検査を足すだけで正しい方を消しておらず、鳴りようがなかった。
VO は検査の**存在**を測る。**配置は錠だけが見ている** — 検査を読みの前に戻す細工は、
冪等ブロック全体を細工テキストに複製することになるので書いていない。
...
- **コントロール VQ は 3 度書き直した。** 1 度目は検査を足すだけで正しい方を消しておらず
  ...
  3 度目 (早い列挙自体を潰す) で FIRED。**細工が欠陥を再現しているかは、走らせるまで
  分からない。**
