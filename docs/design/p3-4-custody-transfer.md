...
- **P2 (A) — その throw が `createLink` の「独自型が失敗したときの生成型フォールバック」に
  ...
  され、恒久的な失敗では `createRelationship` を試してもいないのに `INDETERMINATE` を記録して
  いた (intent はその後で開く)。→ 3 行上の `CaptureIntentFailedException` と同じ形のガードを
  追加。
...
**依然として未処置** (39〜42 巡目の指摘のうち): 冪等性とチェックポイントの `PropertyManager`
