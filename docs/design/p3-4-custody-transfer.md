...
- **記録の訂正**: `PropertyManager.readValue` が `nemaki_conf` を見るのは
  **`cloud.auth.` / `cloud.drive.` / `sso.` / `oidc.` / `saml.` の 5 接頭辞のキーだけ**である。
  したがって `loadFailed` は、通常の `credentialRef` については**そのキーの読みについて何も
  言っていない**。拒否は「どこからも解決できず、かつ設定 DB が落ちている」ときだけなので
  挙動は安全側だが、**「資格情報の読みを一般に守る」という含意は過大**だった。javadoc に明記。
