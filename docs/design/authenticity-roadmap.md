...
>
> **第 3 段 (2026-08-25)**: **書き込み経路に 2 つ残っていた** (初稿は「1 つ」と書いた —
> **これは過大で、レビュー 3 者すべてに指摘された**)。`create()` には
> `create(Map)` と `create(String id, Map)` の 2 オーバーロードがあり、初稿は
> 前者だけを直して**「`create()` は起動後 throw する」とメソッド族全体が直ったかのように
> 書いた**。しかも**同じバッチで新設した 2 つの store
> (`CouchEvidenceLedgerStore.append` / `CouchAnchorReceiptStore.save`) が使うのは後者**で、
> throw することを前提に書かれていた。証拠台帳が全 append を成功と報告する状態を、
> 「直した」と記録していたことになる。両方を直し、**store は戻り値も検査する**ようにした。
>
> **さらに、初稿の直し方が添付の conflict リトライを壊していた。**
> 全例外を `RuntimeException` で包んだので、`AttachmentDaoDelegate` が**型で**捕まえる
> `ConflictException` が catch に入らなくなり、通常の CAS 負けがアップロードの
> 硬い失敗になっていた。フルスイート 5675/0/0 は**これを検出していない**。
> `rethrowIfUnchecked` を切り出し、型を保存するようにして判別テストを足した。
>
> 以下は初稿の記述: `CloudantClientWrapper.create()` は全例外を握って null を返し、
> コメントは「これは初期起動では正常」だった — 起動中は本当で、**それ以降の全リクエストでは嘘**。
> 作られなかった文書が普通の結果として呼び出し側に届き、それを捕まえていたのは
> **添付作成のたびに走る存在確認 GET だけ** (往復台帳の V2)。1 行が嘘をついていたので
> システム全体が検証読みを 1 回余分に払っていた。startup 外は throw に変更。
> 判別テスト 2 本 + 負のコントロール 2 本実測 (握り潰しに戻す / 起動中も throw させる)。
> **これで往復台帳 V2 の 2 条件のうち片方が満たされた** — 残るは
> 「Cloudant の read-your-writes をやめる判断」で、**配備形態の決定なのでオーナー判断**。
>
...
| V2 / V5 | 作成直後の検証読み・多重リトライの削減 | エラー意味論に依存 | **2-1 の後** → **V2 の前提の片方を 2026-08-25 に満たした** (`create()` が起動後 throw)。残る条件は「Cloudant の read-your-writes をやめる判断」= **配備形態の決定なのでオーナー判断**。V5 はリトライを薄くすると `getObject` の TCK が間欠的に落ちる種類で、こちらも判断が要る |
...
| **P1-3** ✅ 第 1 段 2026-08-24 ([`p1-3-evidence-ledger.md`](p1-3-evidence-ledger.md)) — 追記専用エントリ・Merkle checkpoint・inclusion proof・連鎖検証 (改竄 / 削除 / 並べ替え / fork / genesis を別々に報告)。**永続化実装済 2026-08-24 / 生産者 実装済 2026-08-25** (`EvidenceLedgerRecorder` — capture 完了を `CAPTURE_COMPLETED` として追記。**追記の失敗は取込を失敗させないが、黙りもしない** — flag + warning を返し `capturedWithGap` として直接の呼び出し元と REST 応答本文に載る。**ただし定期取得の fetch オーケストレータ 9 本は `warnings()` を読まない**ので、それらの取得サマリには出ずサーバログだけに残る。**台帳は常時オンで `nemaki_evidence_ledger` を自動作成する — 切るスイッチは無い**) (`CouchEvidenceLedgerStore` — 専用 DB `nemaki_evidence_ledger`、journal の purge と寿命を共有しない)。**定期 checkpoint は未配線、外部アンカーは P2**。連鎖ドメインは**リポジトリ単位**に決定 (代償: アンカー費用がドメイン数だけ掛かる。集約根は別増分) | **Tamper-evident evidence ledger** | **journal と evidence を分離する** (外部レビュー + purge 衝突の帰結): 配送用 journal は現行どおり purge 可 (`lineage.retention.days`)、**evidence ledger はアプリケーション層で追記専用**として扱い (物理的な不変性ではない — DB 管理者は到達できる)、期間・法的根拠別に保持し、purge 境界ごとに checkpoint + 外部アンカー + inclusion proof を残す。連鎖の構築は素朴な LeaderElection 流用ではなく、**既存の fenced sequencer (`CouchLineageSequencingStore` — lease generation CAS 持ち) を土台**に: chain domain (repo 単位か全体か) / sequence と連鎖の同一 CAS 確定 / failover 時の fork 検出 / **unsequenced backlog がある間の anchor 禁止** / purge 後 genesis / 「順序 = 確定 sequence 順であって時計順ではない」の明記。但し書き: 連鎖が固定するのは記録された順序 (P1-1 が先)、アンカー以前しか不整合を検出可能にできない (頻度 = 書き直され得る窓) |
| **P1-4** ✅ 2026-08-24 ([`p1-4-authenticity-report.md`](p1-4-authenticity-report.md)) — 7 節を JSON + HTML で集約。**節ごとの verdict と限界を構造で強制** (limits が空なら `Section` を構築できない)。**PDF は作らない** (→ P3-1 の E-ARK)。access 節は構造上 `UNAVAILABLE` (監査証跡は SLF4J でプロセス外へ出る) | **真正性レポート (evidence package)** | 文書 1 件について identity 属性・contentHash と fixity 履歴・custody チェーン (journal 抜粋)・アクセス監査・バージョン系譜・処理環境 (Barrier ダイジェスト) を 1 つの JSON + 人が読む PDF に集約する API/UI。**マーケの主砲** (§5) |
...
| ID | 何を | 具体 |
|---|---|---|
| **P2-0** ✅ 2026-08-25 ([`p2-0-anchor-targets.md`](p2-0-anchor-targets.md)) — 段の型は 2026-08-18 に揃っていたが**誰も呼んでいなかった**。`AnchorService` (未確定 tail の上にアンカーしない / 読めない head を settled 扱いしない / 段ごとの封じ込め)、受領証の永続化、`AnchorWiringConfig`、運用 API を足して環を閉じた。**定期実行は入れない** (アンカー頻度 = 台帳がまだ書き直せる窓の幅) | **アンカー先のプラガブル化** | P1-3 の日次アンカー D の送出先を多重化: Atlas (段 1、既存 sink) / OpenTimestamps (段 2) / RFC 3161 TSA (段 3)。段ごとに独立に有効化 |
...
| **P2-1** ✅ 2026-08-25 — sidecar (`docker/ots/`) と `OpenTimestampsAnchorTarget` は 2026-08-18 実装済み。本日**受領証の永続化**を足して `upgrade()` が成立するようにした (保留中 proof のバイト列が再起動を越えないと、カレンダーは commitment を持つのにこちらは proof を出せない) | **OpenTimestamps アンカー** | D をカレンダーサーバへ送信 (HTTP POST のみ、鍵・ウォレット・暗号資産保有なし)。**公式クライアントは nonce 付き commitment を送る** — 生の D すら外に出ない (privacy 特性として明記・踏襲する)。**証明は二段階** — 送信直後は pending、Bitcoin ブロック確定後 (**数時間かかり得る**) にジョブが `.ots` を upgrade (dead-letter/リトライの既存パターン)。複数カレンダー併用。`.ots` の commitment とブロックヘッダの対応**だけ**は、`.ots` + 信頼できる Bitcoin ブロックヘッダ列で**当社にもカレンダーにも依存せず**照合できる。**それ以上は独立に検証できない** — 捕獲の網羅性・メタデータの真実性・最初のハッシュが正しく取られたことは、この照合では担保されない。証明の意味は「**そのコミットメントがそのブロック時刻までに存在した**」という上限側の存在証明であり、対称な誤差幅の時刻証明ではない。**主語はコミットメントであって記録ではない** |
| **P2-2** ✅ 2026-08-25 — `Rfc3161AnchorTarget` は 2026-08-18 実装済み (BouncyCastle の落とし穴 6 件を明示的に処理)。本日 `AnchorWiringConfig` で設定可能にした。**認定 TSA の選定と契約は運用判断** | RFC 3161 タイムスタンプ (段 3) | 日次アンカー + 必要ならアーカイブ遷移時に TSA トークンを取得し保存。認定 TSA / フリー TSA をプラガブルに。**TSA policy OID・証明書/失効情報 (CRL/OCSP)・nonce・accuracy の保存**まで含めて「検証可能なトークン」とする (長期検証情報は P2-3) |
| **P2-3** 第 2 段 2026-08-26 ([`p2-3-long-term-validity.md`](p2-3-long-term-validity.md)) — 第 1 段でアルゴリズム失効レジストリと renewal 種別判定 (これが無いと renewal 実装があっても発火せず、しかも**破れた後の renewal は元の時刻を救えない**ので監視が先)。第 2 段で **RFC 4998 evidence record の生成と検証を実装** (`ErsRecord` / `ErsVerifier`、§8)。**ただし data object は checkpoint の正規化バイト列であって文書ではない** (`h = H(d)` が checkpoint hash である。**この行は 2026-08-26 まで「data object は checkpoint hash」と書いていた** — p2-3 §8 が「それをやると標準ツールが読めない記録になる」と記録して直した当の誤りで、**同じファイルの 31 行目は直されたのにこの行は残っていた**。2026-08-28 訂正) — RFC 4998 §4.2 の縮約規則 (ソート連結・domain separation 無し) と本製品の RFC 6962 型 Merkle 木は別物なので、こちらの audit path を `reducedHashtree` に載せると標準ツールが必ず落とす。文書と checkpoint を結ぶのは本製品の inclusion proof で、標準 ERS 検証器はそれを見ない。アンカーからの組み立てと SIP 同梱まで実装済み (§9)。**未**: TSA 署名・証明書鎖・失効の検証 (trust anchor を持たない) | 長期有効性 | **timestamp renewal と hash-tree renewal は発火条件が異なる別操作** — 「再タイムスタンプ」の一語で潰さない。ERS は RFC 4998 (ASN.1/CMS) と RFC 6283 (XMLERS) が**表現形式の選択肢**で、採用可否と形式を設計判断として比較。`.ots`・TSA トークンは SIP (P3-1) に同梱して保全 |
...
IP 操作には commons-ip を使用。

**この「対応表が未確認」は 6.3.0 について解けた** (2026-08-27 実測、
[`p3-4-custody-transfer.md`](p3-4-custody-transfer.md) §10)。予想どおり**同梱 commons-ip 版に依存**し、
しかも**版ごとに別のプラグイン**として同居していた:

| RODA 6.3.0 の取込プラグイン | 呼ぶパーサ | 我々の CSIP 2.2.0 |
|---|---|---|
| `EARKSIP2ToAIPPlugin` | `commons_ip2` (METS 1.12) | **取り込む** |
| `EARKSIPToAIPPlugin` | `commons_ip` = **v1** (METS 1.11) | **拒否する** |

v1 API は commons-ip2 の jar の**中に同居している**ので、「同梱版は 2.11.3 だから CSIP 2.x を読む」
とは言えない。**他版の RODA については依然として未確認**で、相互運用の保証にはその版で
実機受入試験が要る。
...
**転送 type の一覧に E-ARK/CSIP 専用のものは無い**。転送 type は
...
**送った SIP 自体の checksum は API から返らない。**

> **この段落は 2 巡目まで 2 つの取り下げ済みの主張を書いていた** (2026-08-27、p3-4 §21 で訂正)。
> **次の一段の指示書がこれなので、直さないと送信経路がそのまま誤る。**
>
> - ~~「接続層で保持する」~~ — それは `sipDigest` を**こちらの記録から埋める**ことで、
>   `CustodyReceiptAssembler` が拒否する「**落ちない検査**」そのものである。
>   正しくは**先方が持っている物から回収して照合する** (p3-4 §14 / §17)
> - ~~「COMPLETE したこと自体がマニフェスト一致の証拠になり」~~ — 状態語から
>   「マニフェストが一致した」を導くのは、受領証がそう記録していない推論である。
>   成立には**先方が `Verify bag` を走らせたこと**が要り、測ったのは
>   `automated` 設定の 1 構成だけで、**受領証はそれを記録しない**

送った bag の `manifest-*.txt` は AIP 内
`{name}-{AIP uuid}/data/objects/metadata/transfers/{name}-{TRANSFER uuid}/` に残るので
`extract_file` で回収できる (2026-08-27 実測、p3-4 §17)。
**AIP ルート直下の同名 manifest は AM 自身のもので、読むと別物を証拠にする。**
