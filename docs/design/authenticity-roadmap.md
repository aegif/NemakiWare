...
3. 現行の取込 lineage snapshot は `sourceSystem/Archetype/ObjectId` 等のみで
   **contentHash・chat*・取込主体を含まず** (`IngestLineageEmitter:51`)、emit の失敗は
   warn して null を返す非致命経路 (:84)。P1-1 の「journal 化の徹底」の実体はここ。**2026-08-19〜20 に対応済み**: 取り逃しは呼び出し元に届き (PR #506)、snapshot は contentState / contentHash / chat* / `executedBy`+`onBehalfOf` を運ぶ (PR #507)。ただし後者は **v1 projection 限定**で v2 に home が無い。
