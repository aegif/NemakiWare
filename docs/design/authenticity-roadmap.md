...
| **P2-3** 第 1 段 2026-08-25 ([`p2-3-long-term-validity.md`](p2-3-long-term-validity.md)) — **ERS は実装していない**。実装したのは RFC 4998 が「範囲外」と明記して肩代わりしてくれない**アルゴリズム失効レジストリと renewal 種別の判定**。これが無いと renewal 実装があっても発火せず、しかも**破れた後の renewal は元の時刻を救えない**ので、監視が先。**ERS の形式は 2026-08-26 に決めた — RFC 4998 (ASN.1/CMS)** ([`p2-3-long-term-validity.md`](p2-3-long-term-validity.md) §7)。**生成・検証は未実装** | 長期有効性 | **timestamp renewal と hash-tree renewal は発火条件が異なる別操作** — 「再タイムスタンプ」の一語で潰さない。ERS は RFC 4998 (ASN.1/CMS) と RFC 6283 (XMLERS) が**表現形式の選択肢**で、採用可否と形式を設計判断として比較。`.ots`・TSA トークンは SIP (P3-1) に同梱して保全 |
