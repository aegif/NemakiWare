...
- **これ以前に送信された値は、この変更では取り消せない。** カタログ側の削除が別途要る。
  → **2026-08-24: 見つける手段を用意した** (§7)。消す判断と操作は運用者側のまま
...
## 7. 既に送ってしまったものをどうするか (2026-08-24)

§1.1 が fail-open として記録した「journal には保持規則があり、**カタログ側には無い**」に
対する回答。**2 つに分けた。**

### 7.1 これから出ていくもの — 構造的に止まっている

P1-1(e) 以降、**v2 record は INTERNAL_ONLY な事実を sink へ運べない**。
`LineageRecord.fromV2` が journal 区画を projection しないので、sink には届く経路が無い。
「送らないよう気をつける」ではなく、渡す型に載っていない。

### 7.2 既に出てしまったもの — 見つける手段を作った

v1 イベントは snapshot map を丸ごと送っていた。したがって
「どの値がもう我々の持ち物でないカタログに在るのか」は実在する問いで、
これまで答える手段が無かった。

```
GET /core/api/v1/admin/lineage-journal/disclosure-exposure?repositoryId=bedroom
```

保存済みイベントを走査し、**現在の表が INTERNAL_ONLY と判定する事実を持つ v1 行**を
返す。応答は eventId / eventKey / occurredAt / processType と**キー名**、および
キー別の件数。

**値は返さない。** この報告は運用者が読んで転送するものなので、値を載せると
「消す手助けをするために、消したいものを広める」ことになる。

**削除はしない — ここでもカタログでも。** 運用者の Purview や Atlas に手を伸ばして
entity を消すのは、その組織のガバナンス判断とツールの領分であり、NemakiWare の
エンドポイントの副作用にしてよいものではない。第一、そのカタログで何がその entity に
依存しているかを製品は知らない。**報告する。運用者が動く。**

**露出の証明でもない。** 行が在るのは「イベントがその事実を運んだ」ことであって、
実際に配送されたかは同じ行の publish status に従う。

**decode できなかった行は数えて返す** (`undecodableRows`)。読めない行は片付けられない
行でもあり、その山の上で「露出 0 件」と報告するのは、同じ fail-open を一段下で
繰り返すことになる。走査上限に当たったときも `truncated: true` を返す。

### 7.3 やらないと決めたこと

- **カタログ側の自動保持・自動削除**。外部システムを製品が定期的に削って回るのは、
  誤りが取り返しのつかない方向に効く。運用者の保持ツールに従う
- **v1 イベントの遡及書き換え**。journal は証拠であり、後から書き換えない
