...
> **⚡ 3.2 以降のワンステップ経路**: ホスト上で WAR をビルドせず、公開イメージを
> pull するだけで 1 VM に立ち上げたい場合は [`deploy/README.md`](../deploy/README.md)
> を参照してください（EC2 user-data / Azure custom-data で自動セットアップ）。
> 本ガイドは手動構築・カスタム構成・運用詳細のリファレンスとして引き続き有効です。
