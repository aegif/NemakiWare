...
### TCK 側に索引待ちを入れた (2026-08-08)

`SearchIndexSettle` を追加し、`QueryForObject` / `QueryLikeTest` / `QueryInFolderTest` を
薄く継承して**最初の query の直前に一度だけ**索引の追いつきを待たせました。
固定 sleep ではなく、**直前に作成したオブジェクトが query で引けるまで**待ちます
(既定 240 秒でタイムアウトし、その場合も query は実行するので本物の失敗は隠れません)。

対象 3 件で足りるのは、これらのテストが**全 create → 全 query の順**だからです。
最後に作ったものが見えれば、それ以前のものも見えています (commit はそれ以前の
追加を全て可視化する)。
...
| | 結果 | 待ち時間 |
|---|---|---|
| `QueryTestGroup` 単独 | **6/6** (109 秒) | 222 / 437 / 1,073 ms |
| `mvn test` 全体 (4,805 件) | **全 green** | 99,118 / 115,456 / 135,376 ms |

**索引の追いつきは平常時 1 秒前後だが、書込みが続いている最中は 100 秒を超える。**
これが元の 60 秒タイムアウトでは足りなかった理由です。単独で 200 件を一気に作る
実験では作成完了の 2.3 秒後に全件検索可能 (87 docs/s) なので、索引そのものが
遅いのではなく、**先行する CRUD グループが残した滞留**を待っています。
`-Dnemakiware.tck.indexSettleTimeoutMs` で調整できます。

なお補助クラスの名前は `...Test` で終わらせてはいけません。Surefire は自前の
テストクラスをその名前で拾い、`AbstractCmisTest` は `@Test` を持つため、
パラメータ無しで単独起動されて `SPI class entry is missing` で落ちます
(上流クラスが無事なのは jar の中に居て走査対象外だから)。

### リリース前に必要なこと

クリーン環境 (`ci-complete-setup.sh`) での確認。上記は全て、私のベンチで
CouchDB 11,000 文書超まで汚れたスタック上の結果です。
