...
CONTROLS = [
    ...
    dict(
        id="KJ",
        what="type registry: a type without BaseId/ParentId is skipped again",
        file="core/src/main/java/jp/aegif/nemaki/cmis/aspect/type/impl/TypeManagerImpl.java",
        find_span=('\t\t\t\tif (subtype.getBaseId() == null || subtype.getParentId() == null) {\n\t\t\t\t\tthrow new IllegalStateException("type definition',
                   'silently omits it");\n\t\t\t\t}'),
        replace='\t\t\t\tif (subtype.getBaseId() == null || subtype.getParentId() == null) {\n\t\t\t\t\tcontinue;\n\t\t\t\t}',
        test="TypeRegistryRefusesBaseOnlyInitTest",
