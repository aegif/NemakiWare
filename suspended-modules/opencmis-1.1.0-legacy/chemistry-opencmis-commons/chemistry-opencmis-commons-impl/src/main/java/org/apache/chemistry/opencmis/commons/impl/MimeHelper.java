...
public final class MimeHelper {
    ...
    public static byte[] getBoundaryFromMultiPart(String value) {
        // CRITICAL DEBUG: Add comprehensive logging for boundary extraction failures
        System.err.println("*** MIME HELPER DEBUG: getBoundaryFromMultiPart() called with value: '" + value + "'");
        
        try {
            HeaderTokenizer tokenizer = new HeaderTokenizer(value, ";", true);
            System.err.println("*** MIME HELPER DEBUG: HeaderTokenizer created successfully");

            // get the first token, which must be an ATOM
            Token token = tokenizer.next();
            System.err.println("*** MIME HELPER DEBUG: First token type: " + token.getType() + ", value: '" + token.getValue() + "'");
            if (token.getType() != Token.ATOM) {
                System.err.println("*** MIME HELPER DEBUG: FAILURE - First token is not ATOM, returning null");
                return null;
            }

            // check content type
            String multipartContentType = token.getValue();
            System.err.println("*** MIME HELPER DEBUG: Multipart content type: '" + multipartContentType + "'");
            if (multipartContentType == null
                    || !(multipartContentType.equalsIgnoreCase("multipart/form-data") || multipartContentType
                            .equalsIgnoreCase("multipart/related"))) {
                System.err.println("*** MIME HELPER DEBUG: FAILURE - Content type not multipart/form-data or multipart/related, returning null");
                return null;
            }

            // the remainder is the parameters
            String remainder = tokenizer.getRemainder();
            System.err.println("*** MIME HELPER DEBUG: Tokenizer remainder: '" + remainder + "'");
            if (remainder != null) {
                Map<String, String> params = new HashMap<String, String>();
                getParameters(remainder, params);
                System.err.println("*** MIME HELPER DEBUG: Parsed parameters: " + params);

                String boundaryStr = params.get("boundary");
                System.err.println("*** MIME HELPER DEBUG: Boundary string from parameters: '" + boundaryStr + "'");
                if (boundaryStr != null && boundaryStr.length() > 0) {
                    try {
                        byte[] boundaryBytes = boundaryStr.getBytes(IOUtils.ISO_8859_1);
                        System.err.println("*** MIME HELPER DEBUG: SUCCESS - Boundary extracted successfully, length: " + boundaryBytes.length);
                        return boundaryBytes;
                    } catch (UnsupportedEncodingException e) {
                        // shouldn't happen...
                        System.err.println("*** MIME HELPER DEBUG: FAILURE - UnsupportedEncodingException: " + e.getMessage());
                        throw new CmisRuntimeException("Unsupported encoding 'ISO-8859-1'", e);
                    }
                } else {
                    System.err.println("*** MIME HELPER DEBUG: FAILURE - Boundary string is null or empty");
                }
            } else {
                System.err.println("*** MIME HELPER DEBUG: FAILURE - Tokenizer remainder is null");
            }
        } catch (ParseException e) {
            System.err.println("*** MIME HELPER DEBUG: FAILURE - ParseException: " + e.getMessage());
            e.printStackTrace();
            return null;
        }

        System.err.println("*** MIME HELPER DEBUG: FAILURE - Reached end of method, returning null");
        return null;
    }
